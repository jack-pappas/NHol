...
let REAL_MUL_AC = 
   ...
   ((m * n) * p = m * (n * p)) /\
   (m * (n * p) = n * (m * p))"), REWRITE_TAC [REAL_MUL_ASSOC; EQT_INTRO(SPEC_ALL REAL_MUL_SYM)]
                                  |> THEN <| AP_THM_TAC
                                  |> THEN <| AP_TERM_TAC
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. (x + y = x) <=> (y = &0)") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. (x + y = y) <=> (x = &0)") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. (x + y = &0) <=> (x = --y)") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. (x + y = &0) <=> (y = --x)") : thm
#endif
...
let REAL_LT_GT = 
    ...
    prove((parse_term @"!x y. x < y ==> ~(y < x)"), REAL_ARITH_TAC)
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. x < y ==> ~(y < x)") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. ~(x = y) <=> x < y \/ y < x") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x. (--x = &0) <=> (x = &0)") : thm
#endif
...
let REAL_ADD_SUB = 
    ...
    prove((parse_term @"!x y. (x + y) - x = y"), REAL_ARITH_TAC)
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. (x + y) - x = y") : thm
#endif
...
let REAL_EQ_SUB_LADD = 
    ...
    prove((parse_term @"!x y z. (x = y - z) <=> (x + z = y)"), REAL_ARITH_TAC)
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y z. (x = y - z) <=> (x + z = y)") : thm
#endif
...
let REAL_POS_NZ = 
    ...
    prove((parse_term @"!x. &0 < x ==> ~(x = &0)"), REAL_ARITH_TAC)
#else
    Choice1Of2 <| Sequent([],parse_term @"!x. &0 < x ==> ~(x = &0)") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x. (abs(x) = &0) <=> (x = &0)")
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x. ~(x = &0) <=> &0 < abs(x)") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x. (abs(x) = x) <=> &0 <= x") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. abs(x - y) < abs(y) ==> ~(x = &0)") : thm
#endif
...
let REAL_ABS_MUL = 
    prove
        ((parse_term @"!x y. abs(x * y) = abs(x) * abs(y)"), 
         ...
         |> THEN 
         <| (DISJ_CASES_TAC(SPEC (parse_term @"y:real") REAL_LE_NEGTOTAL)
             |> THENL 
             <| [ALL_TAC
                 GEN_REWRITE_TAC (RAND_CONV << RAND_CONV) [GSYM REAL_ABS_NEG]])
         |> THEN 
         <| ASSUM_LIST(MP_TAC << MATCH_MP REAL_LE_MUL << end_itlist CONJ << rev)
         ...
         |> THEN 
         <| ASM_REWRITE_TAC 
                [real_abs; REAL_MUL_LNEG; REAL_MUL_RNEG; REAL_NEG_NEG])
...
#else
    Choice1Of2 <| Sequent([], parse_term @"!x y z. (x * y = x * z) <=> (x = &0) \/ (y = z)")
#endif
...
let REAL_INV_INV = 
    prove((parse_term @"!x. inv(inv x) = x"), GEN_TAC
                                             |> THEN 
                                             <| ASM_CASES_TAC
                                                    (parse_term @"x = &0")
                                             |> THEN 
                                             ...
                                             |> THEN 
                                             <| MATCH_MP_TAC REAL_MUL_RINV_UNIQ
                                             |> THEN 
    ...
    prove
        ((parse_term @"!x. &0 < x ==> &0 < inv(x)"), 
         ...
         <| REPEAT_TCL DISJ_CASES_THEN ASSUME_TAC 
                (SPEC (parse_term @"inv(x)") REAL_LT_NEGTOTAL)
         |> THEN <| ASM_REWRITE_TAC []
         |> THENL <| [RULE_ASSUM_TAC(REWRITE_RULE [REAL_INV_EQ_0])
                      |> THEN <| ASM_REWRITE_TAC []
                      DISCH_TAC
                      |> THEN 
                      <| SUBGOAL_THEN (parse_term @"&0 < --(inv x) * x") MP_TAC
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x. &0 < x ==> &0 < inv(x)") : thm
#endif
...
let REAL_INV_NEG = 
    prove
        ((parse_term @"!x. inv(--x) = --(inv x)"), GEN_TAC
                                                  |> THEN 
                                                  <| ASM_CASES_TAC
                                                         (parse_term @"x = &0")
                                                  |> THEN 
                                                  <| ASM_REWRITE_TAC 
                                                         [REAL_NEG_0; REAL_INV_0]
                                                  |> THEN 
                                                  <| MATCH_MP_TAC 
                                                         REAL_MUL_LINV_UNIQ
                                                  |> THEN 
...
let REAL_POW_ADD = 
    prove
        ((parse_term @"!x m n. x pow (m + n) = x pow m * x pow n"), 
         ...
         |> THEN 
         <| ASM_REWRITE_TAC 
                [ADD_CLAUSES; real_pow; REAL_MUL_LID; REAL_MUL_ASSOC])
...
let REAL_LT_IMP_NZ = 
    ...
    prove((parse_term @"!x. &0 < x ==> ~(x = &0)"), REAL_ARITH_TAC)
#else
    Choice1Of2 <| Sequent([],parse_term @"!x. &0 < x ==> ~(x = &0)")
#endif
...
let REAL_LT_LCANCEL_IMP = 
    prove
        ((parse_term @"!x y z. &0 < x /\ x * y < x * z ==> y < z"), 
         ...
         |> THEN 
         <| DISCH_THEN
                (MP_TAC << uncurry CONJ << (MATCH_MP REAL_LT_INV ||>> I) 
                 << CONJ_PAIR)
         |> THEN <| DISCH_THEN(MP_TAC << MATCH_MP REAL_LT_LMUL)
         |> THEN 
         <| POP_ASSUM
                (ASSUME_TAC << MATCH_MP REAL_MUL_LINV << MATCH_MP REAL_LT_IMP_NZ)
...
let REAL_LE_MUL_EQ = 
   ...
   (!x y. &0 < y ==> (&0 <= x * y <=> &0 <= x))"), 
         MESON_TAC 
             [REAL_LE_LMUL_EQ; REAL_LE_RMUL_EQ; REAL_MUL_LZERO; REAL_MUL_RZERO])
let REAL_LT_MUL_EQ = 
    prove
        ((parse_term @"(!x y. &0 < x ==> (&0 < x * y <=> &0 < y)) /\
   (!x y. &0 < y ==> (&0 < x * y <=> &0 < x))"), 
         MESON_TAC 
             [REAL_LT_LMUL_EQ; REAL_LT_RMUL_EQ; REAL_MUL_LZERO; REAL_MUL_RZERO])
...
let REAL_MUL_POS_LT = 
    prove
        ((parse_term @"!x y. &0 < x * y <=> &0 < x /\ &0 < y \/ x < &0 /\ y < &0"), 
         REPEAT STRIP_TAC
         |> THEN 
         <| STRIP_ASSUME_TAC(SPEC (parse_term @"x:real") REAL_LT_NEGTOTAL)
         |> THEN 
         <| STRIP_ASSUME_TAC(SPEC (parse_term @"y:real") REAL_LT_NEGTOTAL)
         |> THEN 
         <| ASM_REWRITE_TAC [REAL_MUL_LZERO; REAL_MUL_RZERO; REAL_LT_REFL]
         |> THEN 
         <| ASSUM_LIST(MP_TAC << MATCH_MP REAL_LT_MUL << end_itlist CONJ)
...
let REAL_MUL_POS_LE = 
    ...
    prove((parse_term @"!x y. &0 <= x * y <=>
        ...
        |> THEN <| REAL_ARITH_TAC)
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. &0 <= x * y <=>
         ...
         <| FIRST_ASSUM
                (fun th -> 
                    GEN_REWRITE_TAC LAND_CONV 
                        [GSYM(MATCH_MP REAL_LE_RMUL_EQ th)])
         |> THEN <| ASM_SIMP_TAC [real_div
                                  GSYM REAL_MUL_ASSOC
                                  REAL_MUL_LINV; REAL_MUL_RID; REAL_LT_IMP_NZ])
    ...
    prove
        ((parse_term @"!x y. &0 < x /\ x < y ==> inv(y) < inv(x)"), 
         ...
         |> THENL <| [MATCH_MP_TAC REAL_LT_MUL
                      ...
                      |> THEN <| REAL_ARITH_TAC
                      SUBGOAL_THEN 
                          (parse_term @"(inv x * x = &1) /\ (inv y * y = &1)") 
                          ASSUME_TAC
                      |> THENL <| [CONJ_TAC
                                   |> THEN <| MATCH_MP_TAC REAL_MUL_LINV
                                   |> THEN 
                                   ...
                                   |> THEN <| REAL_ARITH_TAC
                                   ASM_REWRITE_TAC 
                                       [REAL_MUL_ASSOC; REAL_MUL_LID]
                                   |> THEN 
                                   <| GEN_REWRITE_TAC (LAND_CONV << LAND_CONV) 
                                          [REAL_MUL_SYM]
                                   |> THEN <| ASM_REWRITE_TAC [GSYM 
                                                                   REAL_MUL_ASSOC
                                                               REAL_MUL_RID]]])
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. &0 < x /\ x < y ==> inv(y) < inv(x)")
#endif
...
let REAL_DOWN = 
    prove
        ((parse_term @"!d. &0 < d ==> ?e. &0 < e /\ e < d"), 
         ...
         |> THEN 
         <| ASSUME_TAC
                (MATCH_MP REAL_MUL_LINV (REAL_ARITH(parse_term @"~(&2 = &0)")))
         |> THEN <| CONJ_TAC
         |> THEN <| MATCH_MP_TAC REAL_LT_RCANCEL_IMP
...
let REAL_DOWN2 = 
    prove
        ((parse_term @"!d1 d2. &0 < d1 /\ &0 < d2 ==> ?e. &0 < e /\ e < d1 /\ e < d2"), 
         ...
         |> THEN <| ASM_REWRITE_TAC []
         |> THEN 
         <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"e:real") STRIP_ASSUME_TAC)
...
let REAL_DIV_POW2 = 
    prove((parse_term @"!x m n. ~(x = &0)
           ==> (x pow m / x pow n = if n <= m then x pow (m - n)
                                   ...
                                   |> THEN 
                                   <| UNDISCH_TAC(parse_term @"~(n:num <= m)")
                                   |> THEN <| REWRITE_TAC [NOT_LE]
                                   |> THEN 
                                   <| DISCH_THEN(MP_TAC << MATCH_MP LT_IMP_LE)
                                   |> THEN <| ASM_SIMP_TAC [REAL_POW_SUB])
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!x y. x pow 2 + y pow 2 = &0 <=> x = &0 /\ y = &0") : thm
#endif
        ...
        ((parse_term @"!x n. x pow n = &1 <=> abs(x) = &1 /\ (x < &0 ==> EVEN(n)) \/ n = 0"), 
         ...
         <| FIRST_X_ASSUM
                (DISJ_CASES_THEN SUBST1_TAC 
                 << MATCH_MP
                        (REAL_ARITH(parse_term @"abs x = a ==> x = a \/ x = --a")))
         |> THEN <| ASM_REWRITE_TAC [REAL_POW_NEG; REAL_POW_ONE]
         |> THEN <| REPEAT COND_CASES_TAC
         |> THEN <| ASM_REWRITE_TAC []
         |> THEN <| REAL_ARITH_TAC)
#else
    Choice1Of2 <| Sequent([],parse_term @"!x n. x pow n = &1 <=> abs(x) = &1 /\ (x < &0 ==> EVEN(n)) \/ n = 0") : thm
#endif
...
let REAL_POW_LT2_ODD = 
    prove
        ((parse_term @"!n x y. x < y /\ ODD n ==> x pow n < y pow n"), 
         ...
         |> THENL <| [DISJ_CASES_TAC
                          (REAL_ARITH(parse_term @"&0 <= x \/ &0 < --x"))
                      |> THEN <| ASM_SIMP_TAC [REAL_POW_LT2]
                      |> THEN 
                      <| SUBGOAL_THEN 
                             (parse_term @"&0 < --x pow n /\ &0 <= y pow n") 
                             MP_TAC
...
let REAL_POW_EQ_EQ = 
    prove((parse_term @"!n x y. x pow n = y pow n <=>
          ...
          |> THEN 
          <| FIRST_X_ASSUM
                 (X_CHOOSE_THEN (parse_term @"m:num") SUBST1_TAC 
                  << REWRITE_RULE [EVEN_EXISTS])
    ...
    prove
        ((parse_term @"!x. real_sgn x = x / abs x"), 
         ...
         <| [ASM_REWRITE_TAC [real_div; REAL_MUL_LZERO; REAL_SGN_0]
             ...
             |> THEN 
             <| ASM_SIMP_TAC [real_div
                              GSYM REAL_MUL_ASSOC
                              REAL_ABS_ZERO; REAL_MUL_RINV; REAL_MUL_RID]])
