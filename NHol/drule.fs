...
let (term_match:term list -> term -> term -> instantiation) =
  ...
  let rec term_pmatch lconsts env vtm ctm ((insts,homs) as sofar) =
    ...
    | Abs(vv,vbod),Abs(cv,cbod) ->
        let sofar' = 
          safe_insert (mk_dummy(snd(dest_var cv)),mk_dummy(snd(dest_var vv))) insts,homs
      ...
      try 
        ...
        let ni =
          ...
          if compare cargs pats = 0 then
            ...
            let ginsts = 
              map (fun p -> (if is_var p then p else genvar(type_of p)),p) pats
...
let (term_unify:term list -> term -> term -> instantiation) =
  ...
  let augment_insts(t,v) insts =
    ...
    if t' = v then insts
    elif vfree_in v t' then failwith "augment_insts"
...
let HIGHER_REWRITE_CONV =
  let BETA_VAR =
    let rec BETA_CONVS n =
      if n = 1 then TRY_CONV BETA_CONV 
        ...
        THENC (RATOR_CONV (BETA_CONVS (n - 1))) (TRY_CONV BETA_CONV)
    let rec free_beta v tm =
      if is_abs tm then
        let bv,bod = dest_abs tm
        if v = bv then failwith "unchanged" 
      ...
      let l,r = dest_comb tm
      try let lconv = free_beta v l
          (try let rconv = free_beta v r
               COMB2_CONV lconv rconv
           with Failure _ -> RATOR_CONV lconv)
      with Failure _ -> RAND_CONV (free_beta v r)
    free_beta
  let GINST th =
  ...
  fun ths ->
    let thl = map (GINST << SPEC_ALL) ths in
...
let new_definition tm =
  ...
  let l,r = 
    try dest_eq bod
    with Failure _ -> failwith "new_definition: Not an equation"
  let lv,largs = strip_comb l
  let rtm = 
    try list_mk_abs(largs,r)
    with Failure _ -> failwith "new_definition: Non-variable in LHS pattern"
  ...
  let rvs = filter (not << C mem avs) largs in
