...
let rec DEPTH_BINOP_CONV op conv tm =
  ...
  | Comb(Comb(op',l),r) when op' = op ->
      ...
      let rec lth = DEPTH_BINOP_CONV op conv l
      and rth = DEPTH_BINOP_CONV op conv r
      MK_COMB(AP_TERM op' lth,rth)
  | _ -> conv tm
...
let CACHE_CONV =
    let ALPHA_HACK th =
        ...
        fun tm -> if tm' = tm then th else TRANS (ALPHA tm tm') th
    fun conv ->
