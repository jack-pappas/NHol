...
let MP =
  ...
  let pth =
    ...
    CONJUNCT2 (EQ_MP (SYM th2) (ASSUME <| parse_term "p:bool")) in
  fun ith th ->
...
let SPEC =
  ...
  let pth =
    let th1 = EQ_MP(AP_THM FORALL_DEF <| parse_term "P:A->bool") (ASSUME <| parse_term "(!)(P:A->bool)") in
    let th2 = AP_THM (CONV_RULE BETA_CONV th1) <| parse_term "x:A" in
...
let ISPEC t th =
  let x,_ = 
    try dest_forall(concl th) with Failure _ ->
    failwith "ISPEC: input theorem not universally quantified" in
  let tyins = 
    try type_match (snd(dest_var x)) (type_of t) [] with Failure _ ->
...
let EXISTS =
  ...
  let pth =
    ...
    let th2 = SPEC (parse_term "x:A") (ASSUME <| parse_term "!x:A. P x ==> Q") in
...
let DISJ1 =
  ...
  let Q = parse_term "Q:bool" in
  let pth =
    let th1 = CONV_RULE (RAND_CONV BETA_CONV) (AP_THM OR_DEF <| parse_term "P:bool") in
    let th2 = CONV_RULE (RAND_CONV BETA_CONV) (AP_THM th1 <| parse_term "Q:bool") in
    let th3 = MP (ASSUME <| parse_term "P ==> t") (ASSUME <| parse_term "P:bool") in
    let th4 = GEN (parse_term "t:bool") (DISCH (parse_term "P ==> t") (DISCH (parse_term "Q ==> t") th3)) in
...
let DISJ2 =
  ...
  let pth =
    let th1 = CONV_RULE (RAND_CONV BETA_CONV) (AP_THM OR_DEF <| parse_term "P:bool") in
    let th2 = CONV_RULE (RAND_CONV BETA_CONV) (AP_THM th1 <| parse_term "Q:bool") in
    let th3 = MP (ASSUME <| parse_term "Q ==> t") (ASSUME <| parse_term "Q:bool") in
    let th4 = GEN (parse_term "t:bool") (DISCH (parse_term "P ==> t") (DISCH (parse_term "Q ==> t") th3)) in
    EQ_MP (SYM th2) th4 in
  fun tm th ->
...
let DISJ_CASES =
  ...
  let R = parse_term "R:bool" in
  let pth =
    let th1 = CONV_RULE (RAND_CONV BETA_CONV) (AP_THM OR_DEF <| parse_term "P:bool") in
    let th2 = CONV_RULE (RAND_CONV BETA_CONV) (AP_THM th1 <| parse_term "Q:bool") in
    let th3 = SPEC (parse_term "R:bool") (EQ_MP th2 (ASSUME <| parse_term "P \/ Q")) in
    UNDISCH (UNDISCH th3) in
  fun th0 th1 th2 ->
    try let c1 = concl th1 
        let c2 = concl th2 in
...
let EQF_INTRO =
  ...
  let pth =
    ...
    let th2 = DISCH (parse_term "F") (SPEC P (EQ_MP F_DEF (ASSUME <| parse_term "F"))) in
...
let EQF_ELIM =
  let P = parse_term "P:bool" in
  let pth =
    let th1 = EQ_MP (ASSUME <| parse_term "P = F") (ASSUME <| parse_term "P:bool") in
    let th2 = DISCH P (SPEC (parse_term "F") (EQ_MP F_DEF th1)) in
