...
module Hol_kernel =
    ...
    let vsubst = 
        let rec vsubst ilist tm = 
            ...
            | Abs(v, s) -> 
                ...
                then tm
                    ...
                    if s' == s
                    then tm
                    elif exists (fun (t, x) -> vfree_in v t && vfree_in x s) ilist'
                    then 
                        let v' = variant [s'] v
        ...
        fun theta -> 
            if theta = []
            then (fun tm -> tm)
            elif forall (fun (t, x) -> type_of t = snd(dest_var x)) theta
            then vsubst theta
    ...
    let rec orda env tm1 tm2 = 
        if tm1 == tm2 && forall (fun (x, y) -> x = y) env
        then 0
        else 
            match (tm1, tm2) with
    ...
    let rec term_union l1 l2 = 
        ...
        | (h1 :: t1, h2 :: t2) -> 
            ...
            if c = 0
            then h1 :: (term_union t1 t2)
            elif c < 0
            then h1 :: (term_union t1 l2)
    ...
    let new_basic_definition tm = 
        ...
        | Comb(Comb(Const("=", _), Var(cname, ty)), r) -> 
            if not(freesin [] r)
            then failwith "new_definition: term not closed"
            elif not(subset (type_vars_in_term r) (tyvars ty))
    ...
    let new_basic_type_definition tyname (absname, repname) (Sequent(asl, c)) = 
        ...
        then failwith "new_basic_type_definition: Assumptions in theorem"
        else 
            let P, x = 
                try 
                    dest_comb c
            ...
            if not(freesin [] P)
            then failwith "new_basic_type_definition: Predicate is not closed"
            else 
