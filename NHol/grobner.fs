...
//          let newcps = filter (fun (l,(p,q)) -> not(orthogonal l (fst p) (fst q))) rawcps in
...
//    let l = itlist (itlist (lcm_num << denominator << fst) << snd) cert (num_1) in
...
//    let augment = map (fun (c,m) -> (c,0::m)) in
//    let pols' = map augment pols
//    let pol' = augment pol in
...
//      let eths = filter (is_eq o concl) eths0 in
//      if eths = [] then
...
//           // BINDER_CONV (conv : conv) (tm : term) : thm
//           // funpow (n : int) (f : 'a -> 'a) (x : 'a) : 'a
//
//           let (v1 : conv)         = RAND_CONV BETA2_CONV
//           let (v2 : conv -> conv) = COMB2_CONV v1
//           let (v3 : conv)         = RAND_CONV BETA2_CONV
//           let (v4 : conv)         = BINDER_CONV v3                       // conv = (term -> thm)
//           let (v4a : term -> thm) = BINDER_CONV (RAND_CONV BETA2_CONV)   // thm = Sequent of (term list * term)
//           let v5 = funpow 2 v4a
//           // for v5, v4a causes error: Expecting a term -> term but given a term -> thm.
//
//           CONV_RULE(RAND_CONV (NUM_MULTIPLY_CONV pos)) th2
//       with 
