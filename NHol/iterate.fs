...
let FINITE_INDEX_NUMSEG = 
    prove((parse_term @"!s:A->bool.
        ...
        ?f. (!i j. i IN (1..CARD(s)) /\ j IN (1..CARD(s)) /\ (f i = f j)
          ...
          |> THEN <| DISCH_TAC
          |> THEN <| MP_TAC(ISPECL [(parse_term @"s:A->bool");
                                    (parse_term @"CARD(s:A->bool)")] HAS_SIZE_INDEX)
          |> THEN <| ASM_REWRITE_TAC [HAS_SIZE]
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"f:num->A") STRIP_ASSUME_TAC)
          |> THEN <| EXISTS_TAC(parse_term @"\n. f(n - 1):A")
          |> THEN <| ASM_REWRITE_TAC [EXTENSION; IN_IMAGE; IN_NUMSEG]
          |> THEN <| CONJ_TAC
          |> THENL <| [REWRITE_TAC [ARITH_RULE(parse_term @"1 <= i /\ i <= n <=> ~(i = 0) /\ i - 1 < n")]
                       |> THEN 
                       <| ASM_MESON_TAC [ARITH_RULE(parse_term @"~(x = 0) /\ ~(y = 0) /\ (x - 1 = y - 1) ==> (x = y)")];
                       ASM_MESON_TAC [ARITH_RULE(parse_term @"m < C ==> (m = (m + 1) - 1) /\ 1 <= m + 1 /\ m + 1 <= C");
                                      ARITH_RULE(parse_term @"1 <= i /\ i <= n <=> ~(i = 0) /\ i - 1 < n")]])
...
let DISJOINT_NUMSEG = 
    prove
        ((parse_term @"!m n p q. DISJOINT (m..n) (p..q) <=> n < p \/ q < m \/ n < m \/ q < p"), 
         REWRITE_TAC [DISJOINT; IN_NUMSEG; EXTENSION; IN_INTER; NOT_IN_EMPTY]
         |> THEN <| REPEAT GEN_TAC
         |> THEN <| REWRITE_TAC [DE_MORGAN_THM; NOT_LE]
         |> THEN <| EQ_TAC
         |> THENL <| [MESON_TAC [LT_ANTISYM]
                      ARITH_TAC])

let NUMSEG_ADD_SPLIT = 
    prove
...
let SUBSET_NUMSEG = 
    prove
        ((parse_term @"!m n p q. (m..n) SUBSET (p..q) <=> n < m \/ p <= m /\ n <= q"), 
         REPEAT GEN_TAC
         |> THEN <| REWRITE_TAC [SUBSET; IN_NUMSEG]
         |> THEN <| EQ_TAC
         |> THENL <| [MESON_TAC [LE_TRANS; NOT_LE; LE_REFL]
                      ARITH_TAC])
...
let NUMSEG_LT = 
    prove
        ((parse_term @"!n. {x | x < n} = if n = 0 then {} else 0..(n-1)"), 
         ...
         |> THEN <| COND_CASES_TAC
         |> THEN 
         <| REWRITE_TAC [EXTENSION; IN_NUMSEG; IN_ELIM_THM; NOT_IN_EMPTY]
         |> THEN <| ASM_ARITH_TAC)
...
let NUMSEG_CONV = 
    ...
    let pth_2 = MESON [NUMSEG_LREC; ADD1] (parse_term @"m <= n ==> m..n = m INSERT (SUC m..n)")
    ...
    let rec NUMSEG_CONV tm = 
        choice {
            ...
            if nst <> ns_tm then 
                return! Choice.failwith "NUMSEG_CONV"
            else 
                ...
                if n </ m then 
                    ...
                    return! INST [mt, m_tm] pth_1
                else 
                    let th = 
                        MP_CONV NUM_LE_CONV (INST [mt, m_tm;
                                                   nt, n_tm] pth_2)
                    return! 
                        CONV_RULE (funpow 2 RAND_CONV (LAND_CONV NUM_SUC_CONV
                                                       |> THENC <| NUMSEG_CONV)) th
        }
    NUMSEG_CONV
    ...
    prove
        ((parse_term @"!(<<). (!x y:A. x << y /\ y << x ==> x = y) /\
         ...
         |> THENL <| [INDUCT_TAC
                      ...
                      <| ASM_SIMP_TAC 
                             [SET_RULE
                                  (parse_term @"a IN s ==> (s DELETE a = {} <=> s = {a})")]
         ...
         |> THEN <| INDUCT_TAC
         |> THENL <| [SIMP_TAC 
                          [HAS_SIZE_0; NUMSEG_CLAUSES; ARITH; IMAGE_CLAUSES; 
                           NOT_IN_EMPTY]
                      ALL_TAC]
         ...
         |> THEN 
         <| SUBGOAL_THEN 
                (parse_term @"!i. i IN 1..SUC n <=> i = 1 \/ 1 < i /\ (i - 1) IN 1..n") 
                (fun th -> REWRITE_TAC [EXTENSION; IN_IMAGE; th])
         |> THENL <| [REWRITE_TAC [IN_NUMSEG]
                      |> THEN <| ARITH_TAC
                      ALL_TAC]
         |> THEN <| CONJ_TAC
                      ...
                      <| FIRST_ASSUM
                             (fun th -> 
                                 ONCE_REWRITE_TAC 
                                     [MATCH_MP 
                                          (SET_RULE
                                               (parse_term @"~(b = a) ==> (b IN s <=> b IN (s DELETE a))")) 
                                          th])
                      |> THEN <| ONCE_REWRITE_TAC [COND_RAND]
                      |> THEN <| ASM_REWRITE_TAC [IN_IMAGE; IN_NUMSEG]
                      |> THEN <| EQ_TAC
                      |> THENL <| [ALL_TAC
                                   MESON_TAC []]
   ...
   (!l r. FINITE {x:int | l < x /\ x < r})"), 
         ...
         |> THENL <| [DISCH_TAC
                      ...
                      |> THEN <| REPEAT(MATCH_MP_TAC MONO_FORALL
                                        |> THEN <| GEN_TAC)
                      |> THEN 
                      <| MATCH_MP_TAC(REWRITE_RULE [IMP_CONJ_ALT] FINITE_SUBSET)
                      ...
                      <| ASM_SIMP_TAC 
                             [INT_ARITH
                                  (parse_term @"~(&0 <= r - l:int) ==> ~(l <= x /\ x <= r)")]
                      |> THEN <| ASM_SIMP_TAC [EMPTY_GSPEC; FINITE_EMPTY]
                      |> THEN <| MATCH_MP_TAC FINITE_SUBSET
                      |> THEN 
                      <| EXISTS_TAC
                             (parse_term @"IMAGE (\n. l + &n) (0..num_of_int(r - l))")
                      |> THEN <| ASM_SIMP_TAC [FINITE_IMAGE; FINITE_NUMSEG]
...
let MONOIDAL_AC = prove((parse_term @"!op. monoidal op
        ==> (!a. op (neutral op) a = a) /\
            ...
            (!a b c. op a (op b c) = op b (op a c))"), REWRITE_TAC [monoidal]
                                                       |> THEN <| MESON_TAC [])
...
let SUPPORT_EMPTY = 
    prove
        ((parse_term @"!op f s. (!x. x IN s ==> (f(x) = neutral op)) <=> (support op f s = {})"), 
         REWRITE_TAC [IN_SUPPORT; EXTENSION; IN_ELIM_THM; NOT_IN_EMPTY]
         |> THEN <| MESON_TAC [])
...
let SUPPORT_CLAUSES = 
   ...
   (!f g s.  support op g (IMAGE f s) = IMAGE f (support op (g << f) s))"), 
         REWRITE_TAC 
             [support; EXTENSION; IN_ELIM_THM; IN_INSERT; IN_DELETE; o_THM; 
              IN_IMAGE; NOT_IN_EMPTY; IN_UNION; IN_INTER; IN_DIFF; COND_RAND]
         |> THEN <| REPEAT STRIP_TAC
...
let SUPPORT_DELTA = 
    prove((parse_term @"!op s f a. support op (\x. if x = a then f(x) else neutral op) s =
              ...
              |> THEN <| ASM_REWRITE_TAC [IN_ELIM_THM; NOT_IN_EMPTY])

let FINITE_SUPPORT_DELTA = 
    prove
        ((parse_term @"!op f a. FINITE(support op (\x. if x = a then f(x) else neutral op) s)"), 
         REWRITE_TAC [SUPPORT_DELTA]
         |> THEN <| REPEAT GEN_TAC
         |> THEN <| COND_CASES_TAC
         |> THEN <| SIMP_TAC [FINITE_RULES; FINITE_SUPPORT])
        ...
        ==> (!(f:A->B). iterate op {} f = neutral op) /\
            (!f x s. monoidal op /\ FINITE(support op (f:A->B) s)
                     ==> (iterate op (x INSERT s) f =
                               ...
                               GEN_TAC
                                |> THEN <| STRIP_TAC
                                |> THEN <| ONCE_REWRITE_TAC [AND_FORALL_THM]
                                |> THEN <| GEN_TAC
                                |> THEN 
                                <| MP_TAC
                                ...
                                |> THEN 
                                <| ASM_REWRITE_TAC 
                                       [iterate; SUPPORT_CLAUSES; FINITE_RULES]
                                |> THEN 
                                <| GEN_REWRITE_TAC 
                                       (LAND_CONV << RATOR_CONV << LAND_CONV) 
                                       [COND_RAND]
                                |> THEN 
                                <| ASM_REWRITE_TAC 
...
let ITERATE_UNION = 
    prove(
          (parse_term 
               @"!op. monoidal op ==> !f s t. FINITE s /\ FINITE t /\ DISJOINT s t ==> (iterate op (s UNION t) f = op (iterate op s f) (iterate op t f))"), 
          let lemma = 
              prove
                  (
                   (parse_term 
                        @"(s UNION (x INSERT t) = x INSERT (s UNION t)) /\ (DISJOINT s (x INSERT t) <=> ~(x IN s) /\ DISJOINT s t)"), 
                   SET_TAC [])
          GEN_TAC
          |> THEN <| DISCH_TAC
          |> THEN <| GEN_TAC
          ...
          |> THEN <| ASM_SIMP_TAC [ITERATE_CLAUSES; IN_UNION; UNION_EMPTY; REAL_ADD_RID; lemma; FINITE_UNION]
          |> THEN <| ASM_MESON_TAC [monoidal])
...
let ITERATE_UNION_GEN = 
    prove
        ...
        ==> !(f:A->B) s t. FINITE(support op f s) /\ FINITE(support op f t) /\
         ...
         ONCE_REWRITE_TAC [GSYM ITERATE_SUPPORT]
    ...
    prove
        ...
        ==> !f:A->B s t. FINITE (support op f s) /\
                         ...
                         ==> (op (iterate op (s DIFF t) f) (iterate op t f) =
                              iterate op s f)"),
         ONCE_REWRITE_TAC [GSYM ITERATE_SUPPORT]
         |> THEN <| SIMP_TAC [SUPPORT_CLAUSES; ITERATE_DIFF])
...
let ITERATE_INCL_EXCL = 
    prove((parse_term @"!op. monoidal op
        ==> !s t f. FINITE s /\ FINITE t
          ...
          REPEAT STRIP_TAC
          |> THEN 
          <| ONCE_REWRITE_TAC [SET_RULE(parse_term @"a UNION b = ((a DIFF b) UNION (b DIFF a)) UNION (a INTER b)")]
          |> THEN 
          ...
          |> THEN <| ASM_SIMP_TAC [ITERATE_UNION;
                                   FINITE_UNION;
                                   FINITE_DIFF;
                                   FINITE_INTER;
                                   SET_RULE(parse_term @"DISJOINT (s DIFF s' UNION s' DIFF s) (s INTER s')");
                                   SET_RULE(parse_term @"DISJOINT (s DIFF s') (s' DIFF s)");
                                   SET_RULE(parse_term @"DISJOINT (s DIFF s') (s' INTER s)");
                                   SET_RULE(parse_term @"DISJOINT (s DIFF s') (s INTER s')")]
          |> THEN <| FIRST_X_ASSUM(fun th -> REWRITE_TAC [MATCH_MP MONOIDAL_AC th]))
...
let ITERATE_CLOSED = 
    prove
        ...
        ==> !P. P(neutral op) /\ (!x y. P x /\ P y ==> P (op x y))
         ...
         |> THEN <| REWRITE_TAC [GSYM IN_SUPPORT]
         |> THEN <| COND_CASES_TAC
         |> THEN <| ASM_SIMP_TAC []
         |> THEN <| POP_ASSUM MP_TAC
         |> THEN 
...
let ITERATE_DELTA = 
    prove((parse_term @"!op. monoidal op
        ==> !f a s. iterate op s (\x. if x = a then f(x) else neutral op) =
                    ...
                    |> THEN <| REWRITE_TAC [SUPPORT_DELTA]
                    |> THEN <| REPEAT GEN_TAC
                    |> THEN <| COND_CASES_TAC
...
let ITERATE_IMAGE = 
    prove ((parse_term @"!op. monoidal op
       ==> !f:A->B g:B->C s.
        ...
        ASSUME_TAC
         |> THENL <| [REWRITE_TAC [IMP_CONJ]
                      |> THEN <| MATCH_MP_TAC FINITE_INDUCT_STRONG
                      |> THEN <| ASM_SIMP_TAC [ITERATE_CLAUSES; IMAGE_CLAUSES; FINITE_IMAGE]
                      |> THEN <| REWRITE_TAC [o_THM; IN_INSERT]
    ...
    prove((parse_term @"!op. monoidal op
        ==>  !f:A->B p s.
                ...
                <| [AP_THM_TAC
                    ...
                    FIRST_X_ASSUM
                        (MATCH_MP_TAC 
                         << MATCH_MP(INST_TYPE [aty, bty] ITERATE_IMAGE))]
                |> THEN <| ASM_MESON_TAC [])
    ...
    prove((parse_term @"!op. monoidal op
        ==> !s:A->bool t:A->B->bool x:A->B->C.
            ...
            IMAGE (\j. a,j) (t a) UNION {i,j | i IN s /\ j IN t i}")] 
                    ...
                    |> THEN <| ANTS_TAC
                    |> THENL <| [ASM_SIMP_TAC 
                                     [FINITE_IMAGE; FINITE_PRODUCT_DEPENDENT; 
                                      IN_INSERT]
                                 |> THEN 
                                 <| REWRITE_TAC 
                                        [DISJOINT; EXTENSION; IN_IMAGE; IN_INTER; 
                                         NOT_IN_EMPTY; IN_ELIM_THM; 
                                         EXISTS_PAIR_THM; FORALL_PAIR_THM; 
                                         PAIR_EQ]
                                 |> THEN <| ASM_MESON_TAC []
                                 ALL_TAC]
                    ...
                    <| FIRST_ASSUM
                           (fun th -> 
                               W
                                   (MP_TAC 
                                    << PART_MATCH (Choice.bind lhand << rand) 
    ...
    prove((parse_term @"!op. monoidal op
        ==> !s:A->bool t:B->bool f:A->C g h.
                ...
                |> THENL <| [ASM_MESON_TAC [ITERATE_EQ; o_THM]
                             ...
                             |> THEN 
                             <| FIRST_X_ASSUM
                                    (MATCH_MP_TAC << MATCH_MP ITERATE_IMAGE)
                             |> THEN <| ASM_MESON_TAC []])
    ...
    prove((parse_term @"!op. monoidal op
        ==> !f:A->B s t.
                ...
                ==> iterate op (s UNION t) f =
                    ...
                    |> THEN <| REWRITE_TAC [SUPPORT_CLAUSES]
                    |> THEN 
                    <| FIRST_ASSUM(MATCH_MP_TAC << MATCH_MP ITERATE_UNION)
                    |> THEN 
                    <| ASM_SIMP_TAC 
                           [FINITE_SUPPORT; DISJOINT; IN_INTER; IN_SUPPORT; 
                            EXTENSION]
...
let ITERATE_SUPERSET = 
    prove((parse_term @"!op. monoidal op
        ==> !f:A->B u v.
            ...
            |> THEN <| AP_THM_TAC
            |> THEN <| AP_TERM_TAC
            |> THEN <| REWRITE_TAC [support; EXTENSION; IN_ELIM_THM]
...
let ITERATE_IMAGE_NONZERO = 
    prove((parse_term @"!op. monoidal op
        ==> !g:B->C f:A->B s.
                    ...
                    |> THEN 
                    <| SUBGOAL_THEN 
                           (parse_term @"iterate op s ((g:B->C) << (f:A->B)) = iterate op (IMAGE f s) g") 
                           SUBST1_TAC
                    |> THENL <| [ASM_MESON_TAC []
                                 ALL_TAC]
                ...
                ==> iterate op s (\x. if P x then f x else g x) =
                    op (iterate op {x | x IN s /\ P x} f)
                       ...
                       |> THENL <| [FIRST_ASSUM
                                        (fun th -> 
                                            ASM_SIMP_TAC 
                                                [GSYM(MATCH_MP ITERATE_UNION th)
                                                 FINITE_RESTRICT
                                                 SET_RULE
                                                     (parse_term @"DISJOINT {x | x IN s /\ P x} {x | x IN s /\ ~P x}")])
                                    |> THEN <| AP_THM_TAC
                                    |> THEN <| AP_TERM_TAC
                                    |> THEN <| SET_TAC []
                                    BINOP_TAC
                                    |> THEN 
                                    <| FIRST_ASSUM
                                           (MATCH_MP_TAC << MATCH_MP ITERATE_EQ)
        ...
        ==> !f g:A->B s.
                ...
                ==> iterate op s (\x. op (f x) (g x)) =
                    ...
                    |> THENL <| [CONV_TAC SYM_CONV
                                 ASM_SIMP_TAC [ITERATE_OP; FINITE_UNION]
                                 |> THEN <| BINOP_TAC]
                    |> THEN 
                    <| FIRST_ASSUM(MATCH_MP_TAC << MATCH_MP ITERATE_SUPERSET)
                    |> THEN 
                    <| REWRITE_TAC [support; IN_ELIM_THM; SUBSET; IN_UNION]
                    |> THEN <| ASM_MESON_TAC [monoidal])
    ...
    prove((parse_term @"!op. monoidal op
        ==> !f m n. iterate op (2*m..2*n+1) f =
                    ...
                    |> THENL <| [ASM_SIMP_TAC [num_CONV(parse_term @"1")
                                 ...
                                 <| REWRITE_TAC 
                                        [ARITH_RULE
                                             (parse_term @"2 * m <= SUC 0 <=> m = 0")]
                                 |> THEN <| COND_CASES_TAC
                                 |> THEN <| ASM_REWRITE_TAC [MULT_EQ_0; ARITH]
                                 REWRITE_TAC 
                                     [ARITH_RULE
                                          (parse_term @"2 * SUC n + 1 = SUC(SUC(2 * n + 1))")]
                                 |> THEN 
                                 <| ASM_SIMP_TAC [ITERATE_CLAUSES_NUMSEG]
                                 |> THEN 
                                 <| REWRITE_TAC 
                                        [ARITH_RULE
                                             (parse_term @"2 * m <= SUC(SUC(2 * n + 1)) <=> m <= SUC n")
                                         ARITH_RULE
                                             (parse_term @"2 * m <= SUC(2 * n + 1) <=> m <= SUC n")]
                                 |> THEN <| COND_CASES_TAC
                                 |> THEN <| ASM_REWRITE_TAC []
                                 |> THEN 
                                 <| REWRITE_TAC 
                                        [ARITH_RULE
                                             (parse_term @"2 * SUC n = SUC(2 * n + 1)")
                                         ARITH_RULE
                                             (parse_term @"2 * SUC n + 1 = SUC(SUC(2 * n + 1))")]
                                 |> THEN <| ASM_MESON_TAC [monoidal]])
...
let NSUM_ADD_GEN = 
    prove((parse_term @"!f g s.
       ...
       REWRITE_TAC [GSYM NEUTRAL_ADD
                    GSYM support
                    nsum]
       |> THEN <| MATCH_MP_TAC ITERATE_OP_GEN
       |> THEN <| ACCEPT_TAC MONOIDAL_ADD)
...
let NSUM_LT_ALL = 
    prove((parse_term @"!f g s. FINITE s /\ ~(s = {}) /\ (!x. x IN s ==> f(x) < g(x))
...
let NSUM_POS_BOUND = 
    prove((parse_term @"!f b s. FINITE s /\ nsum s f <= b ==> !x:A. x IN s ==> f x <= b"), 
          GEN_TAC
          |> THEN <| GEN_TAC
          |> THEN <| REWRITE_TAC [IMP_CONJ]
          |> THEN <| MATCH_MP_TAC FINITE_INDUCT_STRONG
          |> THEN <| SIMP_TAC [NSUM_CLAUSES; NOT_IN_EMPTY; IN_INSERT]
          |> THEN <| MESON_TAC [LE_0;
                                ARITH_RULE(parse_term @"0 <= x /\ 0 <= y /\ x + y <= b ==> x <= b /\ y <= b")])
...
let NSUM_BOUND_LT_ALL = 
    prove((parse_term @"!s f b. FINITE s /\ ~(s = {}) /\ (!x. x IN s ==> f(x) < b)
...
let NSUM_EQ_SUPERSET = 
    prove((parse_term @"!f s t:A->bool.
        FINITE t /\ t SUBSET s /\
        (!x. x IN t ==> (f x = g x)) /\
        (!x. x IN s /\ ~(x IN t) ==> (f(x) = 0))
        ==> (nsum s f = nsum t g)"), MESON_TAC [NSUM_SUPERSET; NSUM_EQ])

let NSUM_RESTRICT_SET = 
    prove
        ((parse_term @"!P s f. nsum {x:A | x IN s /\ P x} f = nsum s (\x. if P x then f(x) else 0)"), 
         ONCE_REWRITE_TAC [GSYM NSUM_SUPPORT]
         |> THEN <| REWRITE_TAC [support; NEUTRAL_ADD; IN_ELIM_THM]
         |> THEN <| REWRITE_TAC [MESON [] 
                                     (parse_term @"~((if P x then f x else a) = a) <=> P x /\ ~(f x = a)")
                                 GSYM CONJ_ASSOC]
         |> THEN <| REPEAT GEN_TAC
         |> THEN <| MATCH_MP_TAC NSUM_EQ
         |> THEN <| SIMP_TAC [IN_ELIM_THM])
...
let NSUM_NSUM_RESTRICT = 
    prove
        ...
        ==> (nsum s (\x. nsum {y | y IN t /\ R x y} (\y. f x y)) =
         ...
         REPEAT GEN_TAC
         |> THEN <| SIMP_TAC [NSUM_RESTRICT_SET]
         |> THEN <| DISCH_THEN(fun th -> REWRITE_TAC [MATCH_MP NSUM_SWAP th]))
...
let NSUM_MULTICOUNT_GEN = 
    prove((parse_term @"!R:A->B->bool s t k.
        ...
        ==> (nsum s (\i. (CARD {j | j IN t /\ R i j})) =
             ...
             |> THEN 
             <| EXISTS_TAC
                    (parse_term @"nsum s (\i:A. nsum {j:B | j IN t /\ R i j} (\j. 1))")
             |> THEN <| CONJ_TAC
             |> THENL <| [MATCH_MP_TAC NSUM_EQ
                          |> THEN <| ASM_REWRITE_TAC []
                          |> THEN <| REPEAT STRIP_TAC
                          |> THEN 
                          ...
                          FIRST_ASSUM
                              (fun t -> 
                                  ONCE_REWRITE_TAC 
                                      [MATCH_MP NSUM_NSUM_RESTRICT t])
                          |> THEN <| MATCH_MP_TAC NSUM_EQ
                          |> THEN <| ASM_SIMP_TAC [NSUM_CONST; FINITE_RESTRICT]
...
let NSUM_IMAGE_GEN = 
    prove
        ...
        ==> (nsum s g =
         ...
         |> THEN 
         <| EXISTS_TAC
                (parse_term @"nsum s (\x:A. nsum {y:B | y IN IMAGE f s /\ (f x = y)} (\y. g x))")
         |> THEN <| CONJ_TAC
         |> THENL 
         <| [MATCH_MP_TAC NSUM_EQ
             ...
             |> THEN 
             <| SUBGOAL_THEN 
                    (parse_term @"{y | y IN IMAGE (f:A->B) s /\ (f x = y)} = {(f x)}") 
                    (fun th -> REWRITE_TAC [th; NSUM_SING; o_THM])
             |> THEN <| REWRITE_TAC [EXTENSION; IN_ELIM_THM; IN_SING; IN_IMAGE]
             |> THEN <| ASM_MESON_TAC []
             GEN_REWRITE_TAC (funpow 2 RAND_CONV << ABS_CONV << RAND_CONV) 
                 [GSYM ETA_AX]
...
let NSUM_SUBSET = 
    prove((parse_term @"!u v f. FINITE u /\ FINITE v /\ (!x:A. x IN (u DIFF v) ==> f(x) = 0)
           ==> nsum u f <= nsum v f"),
          REPEAT STRIP_TAC
          |> THEN <| MP_TAC(ISPECL [(parse_term @"f:A->num");
                                    (parse_term @"u INTER v :A->bool")] NSUM_UNION)
          |> THEN <| DISCH_THEN(fun th -> MP_TAC(SPEC (parse_term @"v DIFF u :A->bool") th)
                                          |> THEN <| MP_TAC(SPEC (parse_term @"u DIFF v :A->bool") th))
          |> THEN <| REWRITE_TAC [SET_RULE(parse_term @"(u INTER v) UNION (u DIFF v) = u");
...
let NSUM_IMAGE_NONZERO = 
    ...
    |> THEN <| MATCH_MP_TAC ITERATE_IMAGE_NONZERO
    |> THEN <| REWRITE_TAC [MONOIDAL_ADD])
...
let NSUM_BIJECTION = 
    prove((parse_term @"!f p s:A->bool.
                ...
                ==> nsum s f = nsum s (f << p)"),
                REWRITE_TAC [nsum]
                |> THEN <| MATCH_MP_TAC ITERATE_BIJECTION
                |> THEN <| REWRITE_TAC [MONOIDAL_ADD])
...
let NSUM_NSUM_PRODUCT = 
    prove((parse_term @"!s:A->bool t:A->B->bool x.
        ...
        ==> nsum s (\i. nsum (t i) (x i)) =
            ...
            REWRITE_TAC [nsum]
            |> THEN <| MATCH_MP_TAC ITERATE_ITERATE_PRODUCT
            |> THEN <| REWRITE_TAC [MONOIDAL_ADD])
...
let NSUM_EQ_GENERAL = 
    prove((parse_term @"!s:A->bool t:B->bool f g h.
        ...
        ==> nsum s f = nsum t g"),
        REWRITE_TAC [nsum]
        |> THEN <| MATCH_MP_TAC ITERATE_EQ_GENERAL
        |> THEN <| REWRITE_TAC [MONOIDAL_ADD])
...
let NSUM_EQ_GENERAL_INVERSES = 
    prove((parse_term @"!s:A->bool t:B->bool f g h k.
        ...
        ==> nsum s f = nsum t g"),
        REWRITE_TAC [nsum]
        |> THEN <| MATCH_MP_TAC ITERATE_EQ_GENERAL_INVERSES
        |> THEN <| REWRITE_TAC [MONOIDAL_ADD])
...
let NSUM_INJECTION = 
    prove((parse_term @"!f p s. FINITE s /\
           ...
           ==> nsum s (f << p) = nsum s f"),
           REWRITE_TAC [nsum]
           |> THEN <| MATCH_MP_TAC ITERATE_INJECTION
           |> THEN <| REWRITE_TAC [MONOIDAL_ADD])
...
let NSUM_UNION_NONZERO = 
    prove((parse_term @"!f s t. FINITE s /\ FINITE t /\ (!x. x IN s INTER t ==> f(x) = 0)
           ...
           REWRITE_TAC [nsum
                        GSYM NEUTRAL_ADD]
           |> THEN <| MATCH_MP_TAC ITERATE_UNION_NONZERO
           |> THEN <| REWRITE_TAC [MONOIDAL_ADD])
...
let NSUM_UNIONS_NONZERO = 
    prove((parse_term @"!f s. FINITE s /\ (!t:A->bool. t IN s ==> FINITE t) /\
         ...
         |> THEN <| ANTS_TAC
         |> THENL <| [ASM_MESON_TAC []
                      DISCH_THEN(SUBST_ALL_TAC << SYM)]
         |> THEN <| STRIP_TAC
         |> THEN <| MATCH_MP_TAC NSUM_UNION_NONZERO
         |> THEN <| ASM_SIMP_TAC [FINITE_UNIONS; IN_INTER; IN_UNIONS]
         |> THEN <| ASM_MESON_TAC [])
...
let NSUM_CASES = 
    prove((parse_term @"!s P f g. FINITE s
             ==> nsum s (\x:A. if P x then f x else g x) =
                 ...
                 REWRITE_TAC [nsum
                              GSYM NEUTRAL_ADD]
                 |> THEN <| MATCH_MP_TAC ITERATE_CASES
                 |> THEN <| REWRITE_TAC [MONOIDAL_ADD])
...
let NSUM_LE_NUMSEG = 
    prove((parse_term @"!f g m n. (!i. m <= i /\ i <= n ==> f(i) <= g(i))
...
let NSUM_CLAUSES_NUMSEG = prove((parse_term @"(!m. nsum(m..0) f = if m = 0 then f(0) else 0) /\
   (!m n. nsum(m..SUC n) f = if m <= SUC n then nsum(m..n) f + f(SUC n)
                             ...
                             MP_TAC
                                 (MATCH_MP ITERATE_CLAUSES_NUMSEG MONOIDAL_ADD)
...
let NSUM_CLAUSES_RIGHT = 
    prove
        ((parse_term @"!f m n. 0 < n /\ m <= n ==> nsum(m..n) f = nsum(m..n-1) f + f(n)"), 
         GEN_TAC
         |> THEN <| GEN_TAC
         |> THEN <| INDUCT_TAC
         |> THEN <| SIMP_TAC [LT_REFL; NSUM_CLAUSES_NUMSEG; SUC_SUB1])

let NSUM_PAIR = 
    prove
        ((parse_term @"!f m n. nsum(2*m..2*n+1) f = nsum(m..n) (\i. f(2*i) + f(2*i+1))"), 
         MP_TAC(MATCH_MP ITERATE_PAIR MONOIDAL_ADD)
...
let MOD_NSUM_MOD_NUMSEG = 
    prove((parse_term @"!f a b n.
        ~(n = 0)
...
let CARD_UNIONS = 
    prove((parse_term @"!s:(A->bool)->bool.
        ...
        |> THEN <| ASM_SIMP_TAC [FINITE_UNIONS; FINITE_UNION; INTER_UNIONS]
...
let SUM_UNION = 
    prove((parse_term @"!f s t. FINITE s /\ FINITE t /\ DISJOINT s t
...
let SUM_ADD_GEN = 
    prove((parse_term @"!f g s.
       ...
       REWRITE_TAC [GSYM NEUTRAL_REAL_ADD
                    ...
                    sum]
       |> THEN <| MATCH_MP_TAC ITERATE_OP_GEN
       |> THEN <| ACCEPT_TAC MONOIDAL_REAL_ADD)
...
let SUM_LMUL = 
    prove
        ((parse_term @"!f c s:A->bool. sum s (\x. c * f(x)) = c * sum s f"), 
         ...
         |> THEN 
         <| SIMP_TAC 
                [SUM_CLAUSES; REAL_MUL_RZERO; REAL_MUL_LZERO; REAL_ADD_LDISTRIB])
...
let SUM_POS_BOUND = 
    prove
        ((parse_term @"!f b s. FINITE s /\ (!x. x IN s ==> &0 <= f x) /\ sum s f <= b
         ...
         |> THEN <| REWRITE_TAC [IMP_CONJ]
         |> THEN <| MATCH_MP_TAC FINITE_INDUCT_STRONG
         |> THEN <| SIMP_TAC [SUM_CLAUSES; NOT_IN_EMPTY; IN_INSERT]
         |> THEN 
         <| MESON_TAC 
                [SUM_POS_LE
                 REAL_ARITH
    ...
    prove((parse_term @"!(u:A->real) s.
         ...
         ==> (!i. i IN s ==> u i = &0) \/
             ...
             |> THEN <| REPEAT_TCL DISJ_CASES_THEN ASSUME_TAC (MESON 
                                                                   [REAL_ARITH
                                                                        (parse_term @"(&0 <= --u <=> ~(u > &0)) /\ (&0 <= u <=> ~(u < &0))")] (parse_term @"(?j k:A. j IN s /\ u j < &0 /\ k IN s /\ u k > &0) \/
...
let SUM_DELETE_CASES = 
    prove((parse_term @"!f s a. FINITE s
           ==> sum (s DELETE a) f = if a IN s then sum s f - f(a)
                                    ...
                                    REPEAT STRIP_TAC
                                    |> THEN <| COND_CASES_TAC
                                    |> THEN <| ASM_SIMP_TAC [SET_RULE
                                                                 (parse_term @"~(a IN s) ==> (s DELETE a = s)")
                                                             SUM_DELETE])
...
let SUM_EQ_SUPERSET = 
    prove((parse_term @"!f s t:A->bool.
        ...
        (!x. x IN t ==> (f x = g x)) /\
        (!x. x IN s /\ ~(x IN t) ==> (f(x) = &0))
        ==> (sum s f = sum t g)"), MESON_TAC [SUM_SUPERSET; SUM_EQ])

let SUM_RESTRICT_SET = 
    prove
        ((parse_term @"!P s f. sum {x | x IN s /\ P x} f = sum s (\x. if P x then f x else &0)"), 
         ONCE_REWRITE_TAC [GSYM SUM_SUPPORT]
         |> THEN <| REWRITE_TAC [support; NEUTRAL_REAL_ADD; IN_ELIM_THM]
         |> THEN <| REWRITE_TAC [MESON [] 
                                     (parse_term @"~((if P x then f x else a) = a) <=> P x /\ ~(f x = a)")
                                 GSYM CONJ_ASSOC]
         |> THEN <| REPEAT GEN_TAC
...
let SUM_SUM_RESTRICT = 
    prove
        ...
        ==> (sum s (\x. sum {y | y IN t /\ R x y} (\y. f x y)) =
             ...
             REPEAT GEN_TAC
             |> THEN <| SIMP_TAC [SUM_RESTRICT_SET]
             |> THEN <| DISCH_THEN(fun th -> REWRITE_TAC [MATCH_MP SUM_SWAP th]))
...
let SUM_MULTICOUNT_GEN = 
    prove((parse_term @"!R:A->B->bool s t k.
        ...
        ==> (sum s (\i. &(CARD {j | j IN t /\ R i j})) =
             ...
             |> THEN 
             <| EXISTS_TAC
                    (parse_term @"sum s (\i:A. sum {j:B | j IN t /\ R i j} (\j. &1))")
             |> THEN <| CONJ_TAC
             |> THENL <| [MATCH_MP_TAC SUM_EQ
                          |> THEN <| ASM_REWRITE_TAC []
                          |> THEN <| REPEAT STRIP_TAC
                          |> THEN <| ASM_SIMP_TAC [CARD_EQ_SUM; FINITE_RESTRICT]
                          FIRST_ASSUM
                              (fun th -> 
                                  ONCE_REWRITE_TAC 
                                      [MATCH_MP SUM_SUM_RESTRICT th])
                          |> THEN <| MATCH_MP_TAC SUM_EQ
                          |> THEN <| ASM_SIMP_TAC [SUM_CONST; FINITE_RESTRICT]
...
let SUM_MULTICOUNT = 
    prove((parse_term @"!R:A->B->bool s t k.
        ...
        |> THENL <| [MATCH_MP_TAC SUM_MULTICOUNT_GEN
                     |> THEN <| ASM_REWRITE_TAC []
                     ASM_SIMP_TAC [SUM_CONST; REAL_OF_NUM_MUL]
                     |> THEN <| REWRITE_TAC [MULT_AC]])
...
let SUM_IMAGE_GEN = 
    prove
        ...
        ==> (sum s g =
         ...
         |> THEN 
         <| EXISTS_TAC
                (parse_term @"sum s (\x:A. sum {y:B | y IN IMAGE f s /\ (f x = y)} (\y. g x))")
         |> THEN <| CONJ_TAC
         |> THENL <| [MATCH_MP_TAC SUM_EQ
                      |> THEN <| ASM_REWRITE_TAC []
                      |> THEN <| X_GEN_TAC(parse_term @"x:A")
                      ...
                      <| SUBGOAL_THEN 
                             (parse_term @"{y | y IN IMAGE (f:A->B) s /\ (f x = y)} = {(f x)}") 
                             (fun th -> REWRITE_TAC [th; SUM_SING; o_THM])
                      |> THEN 
                      <| REWRITE_TAC [EXTENSION; IN_ELIM_THM; IN_SING; IN_IMAGE]
                      |> THEN <| ASM_MESON_TAC []
                      GEN_REWRITE_TAC 
                          (funpow 2 RAND_CONV << ABS_CONV << RAND_CONV) 
                          [GSYM ETA_AX]
...
let SUM_UNIONS_NONZERO = 
    prove((parse_term @"!f s. FINITE s /\ (!t:A->bool. t IN s ==> FINITE t) /\
         ...
         |> THEN <| DISCH_THEN(fun th -> STRIP_TAC
                                         |> THEN <| MP_TAC th)
         |> THEN <| ONCE_REWRITE_TAC [IMP_CONJ]
         |> THEN <| ASM_SIMP_TAC [SUM_CLAUSES]
         |> THEN <| ANTS_TAC
         |> THENL <| [ASM_MESON_TAC []
                      DISCH_THEN(SUBST_ALL_TAC << SYM)]
         |> THEN <| STRIP_TAC
         |> THEN <| MATCH_MP_TAC SUM_UNION_NONZERO
         |> THEN <| ASM_SIMP_TAC [FINITE_UNIONS; IN_INTER; IN_UNIONS]
         |> THEN <| ASM_MESON_TAC [])
...
let SUM_LE_INCLUDED = 
    prove((parse_term @"!f:A->real g:B->real s t i.
        ...
        |> THEN 
        <| EXISTS_TAC
               (parse_term @"sum (IMAGE (i:B->A) t) (\y. sum {x | x IN t /\ i x = y} g)")
        |> THEN <| CONJ_TAC
        |> THENL <| [ALL_TAC
                     MATCH_MP_TAC REAL_EQ_IMP_LE
                     |> THEN <| MATCH_MP_TAC(GSYM SUM_IMAGE_GEN)
                     |> THEN <| ASM_REWRITE_TAC []]
        |> THEN <| MATCH_MP_TAC REAL_LE_TRANS
        |> THEN 
        <| EXISTS_TAC
        ...
        |> THENL <| [MATCH_MP_TAC SUM_LE
                     ...
                     |> THENL <| [ASM_REWRITE_TAC [SUM_SING]
                                  ALL_TAC]
                     ALL_TAC]
        |> THEN <| MATCH_MP_TAC SUM_SUBSET_SIMPLE
        |> THEN <| ASM_SIMP_TAC [FINITE_IMAGE]
        |> THEN <| ASM_SIMP_TAC [SUM_POS_LE; FINITE_RESTRICT; IN_ELIM_THM]
        |> THEN <| ASM SET_TAC [])
...
let SUM_POS_EQ_0_NUMSEG = 
    prove((parse_term @"!f m n. (!p. m <= p /\ p <= n ==> &0 <= f(p)) /\ (sum(m..n) f = &0)
...
let SUM_CLAUSES_NUMSEG = 
    prove((parse_term @"(!m. sum(m..0) f = if m = 0 then f(0) else &0) /\
   (!m n. sum(m..SUC n) f = if m <= SUC n then sum(m..n) f + f(SUC n)
                            else sum(m..n) f)"),
                            MP_TAC
                                (MATCH_MP ITERATE_CLAUSES_NUMSEG 
                                     MONOIDAL_REAL_ADD)
...
let SUM_CLAUSES_RIGHT = 
    prove
        ((parse_term @"!f m n. 0 < n /\ m <= n ==> sum(m..n) f = sum(m..n-1) f + f(n)"), 
         GEN_TAC
         |> THEN <| GEN_TAC
         |> THEN <| INDUCT_TAC
         |> THEN <| SIMP_TAC [LT_REFL; SUM_CLAUSES_NUMSEG; SUC_SUB1])

let SUM_PAIR = 
    prove
        ((parse_term @"!f m n. sum(2*m..2*n+1) f = sum(m..n) (\i. f(2*i) + f(2*i+1))"), 
         MP_TAC(MATCH_MP ITERATE_PAIR MONOIDAL_REAL_ADD)
...
let SUM_PARTIAL_SUC = 
    prove((parse_term @"!f g m n.
        sum (m..n) (\k. f(k) * (g(k + 1) - g(k))) =
            ...
            |> THEN <| INDUCT_TAC
            |> THEN <| COND_CASES_TAC
            |> THEN <| ASM_SIMP_TAC [SUM_TRIV_NUMSEG
                                     GSYM NOT_LE]
            |> THEN <| ASM_REWRITE_TAC [SUM_CLAUSES_NUMSEG]
            |> THENL <| [COND_CASES_TAC
                         |> THEN <| ASM_SIMP_TAC []
                         |> THENL <| [REAL_ARITH_TAC; ASM_ARITH_TAC]
                         ALL_TAC]
            |> THEN <| FIRST_X_ASSUM(MP_TAC << GEN_REWRITE_RULE I [LE])
...
let SUM_DIFFS_ALT = 
    prove((parse_term @"!m n. sum(m..n) (\k. f(k + 1) - f(k)) =
          ...
          |> THEN <| SIMP_TAC [SUM_NEG; SUM_DIFFS]
          |> THEN <| COND_CASES_TAC
...
let SUM_COMBINE_R = 
    prove((parse_term @"!f m n p. m <= n + 1 /\ n <= p
             ...
             |> THEN 
             <| REWRITE_TAC 
                    [FINITE_NUMSEG; EXTENSION; IN_INTER; IN_UNION; NOT_IN_EMPTY; 
                     IN_NUMSEG]
             |> THEN <| ASM_ARITH_TAC)
...
let REAL_SUB_POW = 
    prove
     ...
     ==> n - 1 - x = n - (x + 1) /\ SUC(n - 1 - x) = n - x")]
         ...
         |> THEN 
         <| SIMP_TAC 
                [SUB_0; SUB_ADD; SUB_REFL; real_pow; REAL_MUL_LID; REAL_MUL_RID])
...
let REAL_SUB_POW_L1 = 
    prove
        ((parse_term @"!x n. 1 <= n ==> &1 - x pow n = (&1 - x) * sum(0..n-1) (\i. x pow i)"), 
         ...
         |> THEN <| SIMP_TAC [REAL_SUB_POW_R1]
         |> THEN <| REWRITE_TAC [REAL_MUL_LNEG])
...
let REAL_SUB_POLYFUN = 
    ...
    ==> sum(0..n) (\i. a i * x pow i) - sum(0..n) (\i. a i * y pow i) =
        ...
        REPEAT STRIP_TAC
        |> THEN <| REWRITE_TAC [GSYM SUM_SUB_NUMSEG
                                GSYM REAL_SUB_LDISTRIB]
        |> THEN 
        <| GEN_REWRITE_TAC LAND_CONV [MATCH_MP SUM_CLAUSES_LEFT (SPEC_ALL LE_0)]
        |> THEN 
        <| REWRITE_TAC [REAL_SUB_REFL; real_pow; REAL_MUL_RZERO; REAL_ADD_LID]
...
let REAL_POLYFUN_ROOTBOUND = 
    prove
        ((parse_term @"!n c. ~(!i. i IN 0..n ==> c i = &0)
         ...
         |> THEN <| INDUCT_TAC
         |> THENL 
         <| [REWRITE_TAC [NUMSEG_SING; IN_SING; UNWIND_THM2; SUM_CLAUSES_NUMSEG]
             |> THEN 
             <| SIMP_TAC 
             ...
             |> THENL <| [ASM_SIMP_TAC 
                              [SUM_CLAUSES_NUMSEG; LE_0; REAL_MUL_LZERO; 
                               REAL_ADD_RID]
                          |> THEN <| REWRITE_TAC [LE; LEFT_OR_DISTRIB]
                          |> THEN <| DISJ2_TAC
                          |> THEN <| FIRST_X_ASSUM MATCH_MP_TAC
                          |> THEN <| ASM_MESON_TAC [IN_NUMSEG; LE]
                          ASM_CASES_TAC
                              (parse_term @"{x | sum (0..SUC n) (\i. c i * x pow i) = &0} = {}")
                          |> THEN 
                          <| ASM_REWRITE_TAC [FINITE_RULES; CARD_CLAUSES; LE_0]
                          |> THEN 
                          <| FIRST_X_ASSUM
                                 (MP_TAC 
                                  << GEN_REWRITE_RULE I [GSYM MEMBER_NOT_EMPTY])
                          ...
                          <| DISCH_THEN
                                 (fun th -> 
                                     REWRITE_TAC [th; REAL_ENTIRE; REAL_SUB_0])
                          |> THEN 
                          <| REWRITE_TAC 
                                 [SET_RULE
                                      (parse_term @"{x | x = c \/ P x} = c INSERT {x | P x}")]
                          |> THEN <| MATCH_MP_TAC(MESON 
                                                      [FINITE_INSERT
                                                       CARD_CLAUSES
                                                       ARITH_RULE
                                                           (parse_term @"x <= n ==> SUC x <= SUC n /\ x <= SUC n")] (parse_term @"FINITE s /\ CARD s <= n
...
let REAL_POLYFUN_EQ_CONST = 
    prove((parse_term @"!n c k. (!x. sum(0..n) (\i. c i * x pow i) = k) <=>
           ...
           |> THEN <| CONJ_TAC
           |> THENL <| [SIMP_TAC 
                            [SUM_CLAUSES_LEFT; LE_0; real_pow; REAL_MUL_RID]
                        |> THEN 
                        <| REWRITE_TAC 
                               [REAL_ARITH
                        ...
                        REWRITE_TAC [REAL_POLYFUN_EQ_0; IN_NUMSEG; LE_0]
                        |> THEN 
