...
let TOPOLOGICAL_SORT = 
    prove
        ((parse_term @"!(<<). (!x y:A. x << y /\ y << x ==> x = y) /\
         ...
         <| SUBGOAL_THEN 
                (parse_term 
                     "!i. i IN 1..SUC n <=> i = 1 \/ 1 < i /\ (i - 1) IN 1..n") 
...
let ITERATE_IMAGE_NONZERO = 
    prove((parse_term @"!op. monoidal op
        ==> !g:B->C f:A->B s.
                    ...
                    <| SUBGOAL_THEN 
                           (parse_term 
                                "iterate op s ((g:B->C) << (f:A->B)) = iterate op (IMAGE f s) g") 
                           SUBST1_TAC
                    |> THENL <| [ASM_MESON_TAC []
                                 ALL_TAC]
                                        ...
                                        (fun th -> 
                                            ASM_SIMP_TAC 
                                                [GSYM(MATCH_MP ITERATE_UNION th)
                                                 ...
                                                 SET_RULE
                                                     (parse_term 
                                                          "DISJOINT {x | x IN s /\ P x} {x | x IN s /\ ~P x}")])
                                    |> THEN <| AP_THM_TAC
                                    |> THEN <| AP_TERM_TAC
                                    |> THEN <| SET_TAC []
                                    BINOP_TAC
        ...
        ==> !f m n. iterate op (2*m..2*n+1) f =
                    ...
                    |> THENL <| [ASM_SIMP_TAC [num_CONV(parse_term @"1")
                                 ...
                                 <| REWRITE_TAC 
                                        [ARITH_RULE
                                             (parse_term 
                                                  "2 * m <= SUC(SUC(2 * n + 1)) <=> m <= SUC n")
                                         ARITH_RULE
                                             (parse_term 
                                                  "2 * m <= SUC(2 * n + 1) <=> m <= SUC n")]
                                 |> THEN <| COND_CASES_TAC
                                 |> THEN <| ASM_REWRITE_TAC []
                                 |> THEN 
                                 <| REWRITE_TAC 
    ...
    prove
        ((parse_term 
         ...
         |> THEN <| REWRITE_TAC [MESON [] 
                                     (parse_term 
                                          "~((if P x then f x else a) = a) <=> P x /\ ~(f x = a)")
                                 GSYM CONJ_ASSOC]
         |> THEN <| REPEAT GEN_TAC
         |> THEN <| MATCH_MP_TAC NSUM_EQ
         |> THEN <| SIMP_TAC [IN_ELIM_THM])
...
let NSUM_MULTICOUNT_GEN = 
    prove((parse_term @"!R:A->B->bool s t k.
        ...
        ==> (nsum s (\i. (CARD {j | j IN t /\ R i j})) =
             ...
             <| EXISTS_TAC
                    (parse_term 
                         "nsum s (\i:A. nsum {j:B | j IN t /\ R i j} (\j. 1))")
             |> THEN <| CONJ_TAC
...
let NSUM_IMAGE_GEN = 
    prove
        ...
        ==> (nsum s g =
         ...
         <| EXISTS_TAC
                (parse_term 
                     "nsum s (\x:A. nsum {y:B | y IN IMAGE f s /\ (f x = y)} (\y. g x))")
         |> THEN <| CONJ_TAC
         |> THENL 
         <| [MATCH_MP_TAC NSUM_EQ
             ...
             <| SUBGOAL_THEN 
                    (parse_term 
                         "{y | y IN IMAGE (f:A->B) s /\ (f x = y)} = {(f x)}") 
...
let SUM_MULTICOUNT_GEN = 
    prove((parse_term @"!R:A->B->bool s t k.
        ...
        ==> (sum s (\i. &(CARD {j | j IN t /\ R i j})) =
             ...
             <| EXISTS_TAC
                    (parse_term 
                         "sum s (\i:A. sum {j:B | j IN t /\ R i j} (\j. &1))")
             |> THEN <| CONJ_TAC
...
let SUM_IMAGE_GEN = 
    prove
        ...
        ==> (sum s g =
         ...
         <| EXISTS_TAC
                (parse_term 
                     "sum s (\x:A. sum {y:B | y IN IMAGE f s /\ (f x = y)} (\y. g x))")
         |> THEN <| CONJ_TAC
         |> THENL <| [MATCH_MP_TAC SUM_EQ
                      |> THEN <| ASM_REWRITE_TAC []
                      |> THEN <| X_GEN_TAC(parse_term @"x:A")
...
let SUM_LE_INCLUDED = 
    prove((parse_term @"!f:A->real g:B->real s t i.
        ...
        <| EXISTS_TAC
               (parse_term 
                    "sum (IMAGE (i:B->A) t) (\y. sum {x | x IN t /\ i x = y} g)")
        |> THEN <| CONJ_TAC
        |> THENL <| [ALL_TAC
...
let REAL_POLYFUN_EQ_CONST = 
    prove((parse_term @"!n c k. (!x. sum(0..n) (\i. c i * x pow i) = k) <=>
           ...
           <| EXISTS_TAC
                  (parse_term 
                       "!x. sum(0..n) (\i. (if i = 0 then c 0 - k else c i) * x pow i) = &0")
           |> THEN <| CONJ_TAC
           |> THENL <| [SIMP_TAC 
