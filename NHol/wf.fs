...
let WF_DCHAIN = 
    prove((parse_term @"WF(<<) <=> ~(?s:num->A. !n. s(SUC n) << s(n))"), 
          REWRITE_TAC [WF;
          ...
          |> THENL <| [POP_ASSUM(MP_TAC << REWRITE_RULE [NOT_IMP])
                       |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"a:A")) ASSUME_TAC)
                       |> THEN <| SUBGOAL_THEN (parse_term @"!x:A. ?y. P(x) ==> P(y) /\ y << x") MP_TAC
                       |> THENL <| [ASM_MESON_TAC [];
                                    REWRITE_TAC [SKOLEM_THM]]
                       |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"f:A->A") STRIP_ASSUME_TAC)
                       |> THEN <| CHOOSE_TAC
                                      (prove_recursive_functions_exist num_RECURSION 
                                           (parse_term @"(s(0) = a:A) /\ (!n. s(SUC n) = f(s n))"))
                       |> THEN <| EXISTS_TAC(parse_term @"s:num->A")
                       |> THEN <| ASM_REWRITE_TAC []
                       |> THEN <| SUBGOAL_THEN (parse_term @"!n. P(s n) /\ s(SUC n):A << s(n)") (fun th -> ASM_MESON_TAC [th])
                       |> THEN <| INDUCT_TAC
                       |> THEN <| ASM_REWRITE_TAC []
                       |> THEN <| ASM_MESON_TAC [];
...
let WF_UREC_WF = 
    ...
    ==> WF(<<)"),
          ...
          |> THEN <| GEN_TAC
          |> THEN <| DISCH_TAC
          |> THEN <| FIRST_X_ASSUM(MP_TAC << SPEC(parse_term @"\f x. P(x:A) \/ !z:A. z << x ==> f(z)"))
          |> THEN <| REWRITE_TAC []
...
let WF_REC_INVARIANT = 
    ...
    ==> !H S. (!f g x. (!z. z << x ==> (f z = g z) /\ S z (f z))
              ...
              ==> ?f:A->B. !x. (f x = H f x)"),
          let lemma = 
              prove_inductive_relations_exist(parse_term @"!f:A->B x. (!z. z << x ==> R z (f z)) ==> R x (H f x)")
          REWRITE_TAC [WF_IND]
          |> THEN <| REPEAT STRIP_TAC
          |> THEN <| X_CHOOSE_THEN (parse_term @"R:A->B->bool") STRIP_ASSUME_TAC lemma
          |> THEN <| SUBGOAL_THEN (parse_term @"!x:A. ?!y:B. R x y") (fun th -> ASM_MESON_TAC [th])
          |> THEN <| FIRST_X_ASSUM MATCH_MP_TAC
          |> THEN <| REPEAT STRIP_TAC
          |> THEN <| FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC BINDER_CONV [th])
          |> THEN <| SUBGOAL_THEN (parse_term @"!x:A y:B. R x y ==> S x y") MP_TAC
          |> THEN <| ASM_MESON_TAC [])
...
let WF_REC_WF = 
    ...
    ==> WF(<<)"),
          ...
          |> THEN <| SUBGOAL_THEN (parse_term @"!n. (x:num->A)(@m. x(m) << x(n)) << x(n)") ASSUME_TAC
          ...
          |> THENL <| [REPEAT STRIP_TAC
                       |> THEN <| COND_CASES_TAC
                       |> THEN <| REWRITE_TAC []
                       |> THEN <| FIRST_ASSUM(X_CHOOSE_THEN (parse_term @"p:num") SUBST_ALL_TAC)
                       |> THEN <| AP_TERM_TAC
                       |> THEN <| FIRST_ASSUM MATCH_MP_TAC
                       |> THEN <| FIRST_ASSUM MATCH_ACCEPT_TAC;
                       ALL_TAC]
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"f:A->num") MP_TAC)
          |> THEN <| DISCH_THEN(MP_TAC << GEN(parse_term @"n:num") << SPEC(parse_term @"(x:num->A) n"))
          |> THEN <| SUBGOAL_THEN (parse_term @"!n. ?p. (x:num->A) n = x p") (fun th -> REWRITE_TAC [th])
          |> THENL <| [MESON_TAC [];
                       DISCH_TAC]
          |> THEN <| SUBGOAL_THEN (parse_term @"!n:num. ?k. f(x(k):A) < f(x(n))") ASSUME_TAC
          |> THENL <| [GEN_TAC
...
let WF_MEASURE_GEN = 
    prove((parse_term @"!m:A->B. WF(<<) ==> WF(\x x'. m x << m x')"), 
          GEN_TAC
          |> THEN <| REWRITE_TAC [WF_IND]
          |> THEN <| REPEAT STRIP_TAC
          |> THEN <| FIRST_ASSUM(MP_TAC << SPEC(parse_term @"\y:B. !x:A. (m(x) = y) ==> P x"))
          |> THEN <| UNDISCH_TAC(parse_term @"!x. (!y. (m:A->B) y << m x ==> P y) ==> P x")
          |> THEN <| REWRITE_TAC []
          |> THEN <| MESON_TAC [])
...
let WF_LEX_DEPENDENT = 
    ...
    prove((parse_term @"!R:A->A->bool S:A->B->B->bool. WF(R) /\ (!a. WF(S a))
          ...
          |> THEN <| MAP_EVERY X_GEN_TAC [(parse_term @"a0:A");
                                          (parse_term @"b0:B")]
          |> THEN <| DISCH_TAC
          |> THEN <| FIRST_X_ASSUM(MP_TAC << SPEC(parse_term @"\a:A. ?b:B. P(a,b)"))
          |> THEN <| REWRITE_TAC [LEFT_IMP_EXISTS_THM]
          |> THEN <| DISCH_THEN(MP_TAC << SPECL [(parse_term @"a0:A");
                                                 (parse_term @"b0:B")])
          |> THEN <| ASM_REWRITE_TAC []
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"a:A") (CONJUNCTS_THEN2 MP_TAC ASSUME_TAC))
          |> THEN <| DISCH_THEN(X_CHOOSE_TAC(parse_term @"b1:B"))
          |> THEN <| FIRST_X_ASSUM(MP_TAC << SPECL [(parse_term @"a:A");
                                                    (parse_term @"\b. (P:A#B->bool) (a,b)")])
          |> THEN <| REWRITE_TAC [LEFT_IMP_EXISTS_THM]
          |> THEN <| DISCH_THEN(MP_TAC << SPEC(parse_term @"b1:B"))
          |> THEN <| ASM_REWRITE_TAC []
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"b:B") (CONJUNCTS_THEN2 MP_TAC ASSUME_TAC))
          |> THEN <| DISCH_TAC
          ...
          |> THEN <| ASM_MESON_TAC [])
#else
    Choice.result <| Sequent([], parse_term @"!R:A->A->bool S:A->B->B->bool. WF(R) /\ (!a. WF(S a)) 
...
let WF_POINTWISE = 
    ...
    prove((parse_term @"WF((<<) :A->A->bool) /\ WF((<<<) :B->B->bool)
      ==> WF(\(x1,y1) (x2,y2). x1 << x2 /\ y1 <<< y2)"),
        ...
        |> THEN <| EXISTS_TAC(parse_term @"\(x1,y1) (x2,y2). x1 << x2 \/ (x1:A = x2) /\ (y1:B) <<< (y2:B)")
        |> THEN <| CONJ_TAC
        |> THENL <| [REWRITE_TAC [FORALL_PAIR_THM]
                     ...
                     |> THEN <| ASM_REWRITE_TAC []])
#else
    Sequent([],parse_term @"WF((<<) :A->A->bool) /\ WF((<<<) :B->B->bool) ==> WF(\(x1,y1) (x2,y2). x1 << x2 /\ y1 <<< y2)")
#endif
...
let WF_REC_num = 
    prove((parse_term @"!H. (!f g n. (!m. m < n ==> (f m = g m)) ==> (H f n = H g n))
...
let WF_REFL = 
    prove((parse_term @"!x:A. WF(<<) ==> ~(x << x)"), 
          GEN_TAC
          |> THEN <| REWRITE_TAC [WF]
          |> THEN <| DISCH_THEN(MP_TAC << SPEC(parse_term @"\y:A. y = x"))
          |> THEN <| REWRITE_TAC []
          |> THEN <| MESON_TAC [])
...
let WF_REC_TAIL = 
    prove((parse_term @"!P g h. ?f:A->B. !x. f x = if P(x) then f(g x) else h x"), 
          let lemma1 = prove((parse_term @"~(P 0) ==> ((?n. P(SUC n)) <=> (?n. P(n)))"), MESON_TAC [num_CASES; NOT_SUC])
          let lemma2 = 
              prove((parse_term @"(P 0) ==> ((!m. m < n ==> P(SUC m)) <=> (!m. m < SUC n ==> P(m)))"), 
                    REPEAT(DISCH_TAC
                           |> ORELSE <| EQ_TAC)
                    |> THEN <| INDUCT_TAC
                    |> THEN <| ASM_MESON_TAC [LT_SUC; LT_0])
          REPEAT GEN_TAC
          |> THEN <| MP_TAC(GEN (parse_term @"x:A") (ISPECL [(parse_term @"x:A");
                                                             (parse_term @"\y:A n:num. g(y):A")] num_RECURSION))
          |> THEN <| REWRITE_TAC [SKOLEM_THM; FORALL_AND_THM]
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"s:A->num->A") STRIP_ASSUME_TAC)
          ...
          |> THEN <| SUBGOAL_THEN (parse_term @"!n x:A. s (g x) n :A = s x (SUC n)") ASSUME_TAC
          |> THENL <| [INDUCT_TAC
                       |> THEN <| ASM_REWRITE_TAC [];
                       UNDISCH_THEN (parse_term @"!x:A n. s x (SUC n) :A = g (s x n)") (K ALL_TAC)]
...
let WF_INDUCT_TAC = 
    let qqconv = 
        let pth = prove((parse_term @"(!x. P x ==> !y. Q x y) <=> !y x. P x ==> Q x y"), MESON_TAC [])
        GEN_REWRITE_CONV I [pth]
    
    let eqconv = 
        let pth = prove((parse_term @"(!m. P m ==> (m = e) ==> Q) <=> (P e ==> Q)"), MESON_TAC [])
        REWR_CONV pth

    let rec qqconvs tm = 
        (qqconv |> THENC <| BINDER_CONV qqconvs) tm
        |> Choice.bindError (fun _ -> eqconv tm)
    ...
    fun tm (asl, w as gl) ->
        choice {
            ...
            let gv = genvar ty1
            let! tm2 = mk_eq(gv, tm)
            let! tm3 = mk_imp(tm2, w)
            let! pat = list_mk_forall(gv :: fvs, tm3)
