...
let DIST_ELIM_THM = 
    prove((parse_term @"P(dist(x,y)) <=> !d. ((x = y + d) ==> P(d)) /\ ((y = x + d) ==> P(d))"), 
          DISJ_CASES_TAC(SPECL [(parse_term @"x:num");
                                (parse_term @"y:num")] LE_CASES)
          |> THEN <| POP_ASSUM(X_CHOOSE_THEN (parse_term @"e:num") SUBST1_TAC << REWRITE_RULE [LE_EXISTS])
...
let DIST_LE_CASES, DIST_ADDBOUND, DIST_TRIANGLE, DIST_ADD2, DIST_ADD2_REV = 
    ...
    let DIST_LE_CASES = 
        prove((parse_term @"!m n p. dist(m,n) <= p <=> (m <= n + p) /\ (n <= m + p)"), 
              REPEAT GEN_TAC
              |> THEN <| REPEAT DIST_ELIM_TAC
              |> THEN <| REWRITE_TAC [GSYM ADD_ASSOC;
                                      LE_ADD;
                                      LE_ADD_LCANCEL])
    let DIST_ADDBOUND = 
        prove((parse_term @"!m n. dist(m,n) <= m + n"), 
              REPEAT GEN_TAC
              |> THEN <| DIST_ELIM_TAC
...
let BOUNDS_LINEAR = 
    prove((parse_term @"!A B C. (!n. A * n <= B * n + C) <=> A <= B"), 
          REPEAT GEN_TAC
          |> THEN <| EQ_TAC
          |> THENL <| [CONV_TAC CONTRAPOS_CONV
                       |> THEN <| REWRITE_TAC [NOT_LE]
                       |> THEN <| DISCH_THEN(CHOOSE_THEN SUBST1_TAC << REWRITE_RULE [LT_EXISTS])
                       |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB; LE_ADD_LCANCEL]
                       |> THEN <| DISCH_THEN(MP_TAC << SPEC(parse_term @"SUC C"))
                       |> THEN <| REWRITE_TAC [NOT_LE; MULT_CLAUSES; ADD_CLAUSES; LT_SUC_LE]
                       |> THEN <| REWRITE_TAC [ADD_ASSOC; LE_ADDR];
                       DISCH_THEN(CHOOSE_THEN SUBST1_TAC << REWRITE_RULE [LE_EXISTS])
    ...
    prove((parse_term @"!P. (?B. !n. P(n) <= B) <=>
       (?A B. !n. n * P(n) <= A * n + B)"),
          ...
          |> THENL <| [MAP_EVERY EXISTS_TAC [(parse_term @"B:num");
                       ...
                       |> THEN <| GEN_REWRITE_TAC RAND_CONV [ADD_SYM]
                       |> THEN <| GEN_REWRITE_TAC (RAND_CONV << ONCE_DEPTH_CONV) [MULT_SYM]
                       |> THEN <| REWRITE_TAC [GSYM ADD_ASSOC;
                                               LE_ADD_LCANCEL]
                       |> THEN <| MATCH_MP_TAC LE_TRANS
    ...
    prove((parse_term @"!P A B. (P 0 0 = 0) /\ (!m n. P m n <= A * (m + n) + B) ==>
       (?B. !m n. P m n <= B * (m + n))"),
          ...
          |> THENL <| [RULE_ASSUM_TAC(REWRITE_RULE [ADD_EQ_0])
                       ...
                       |> THEN <| SPEC_TAC((parse_term @"m + n"), (parse_term @"p:num"))
                       |> THEN <| INDUCT_TAC
                       |> THEN <| REWRITE_TAC [MULT_CLAUSES; LE_ADD]])

let BOUNDS_IGNORE = 
    prove((parse_term @"!P Q. (?B. !i. P(i) <= Q(i) + B) <=>
         (?B N. !i. N <= i ==> P(i) <= Q(i) + B)"),
          REPEAT GEN_TAC
          |> THEN <| EQ_TAC
          |> THEN <| STRIP_TAC
          |> THENL <| [EXISTS_TAC(parse_term @"B:num")
                       |> THEN <| ASM_REWRITE_TAC [];
                       POP_ASSUM MP_TAC
                       ...
                       |> THENL <| [REWRITE_TAC [LE_0]
                                    ...
                                    |> THENL <| [MATCH_MP_TAC LE_TRANS
                                                 |> THEN <| EXISTS_TAC(parse_term @"Q(i:num) + B")
                                                 |> THEN <| REWRITE_TAC [LE_ADD; ADD_ASSOC]
                                                 |> THEN <| FIRST_ASSUM MATCH_MP_TAC
                                                 |> THEN <| ASM_REWRITE_TAC [];
...
let NADD_BOUND = 
    prove((parse_term @"!x. ?A B. !n. fn x n <= A * n + B"), 
          GEN_TAC
          |> THEN <| X_CHOOSE_TAC (parse_term @"B:num") (SPEC (parse_term @"x:nadd") NADD_CAUCHY)
          ...
          |> THEN <| REWRITE_TAC [LEFT_ADD_DISTRIB; RIGHT_ADD_DISTRIB; MULT_CLAUSES]
          |> THEN <| REWRITE_TAC [ADD_AC; MULT_AC])

let NADD_MULTIPLICATIVE = 
    prove((parse_term @"!x. ?B. !m n. dist(fn x (m * n),m * fn x n) <= B * m + B"), 
          GEN_TAC
          |> THEN <| X_CHOOSE_TAC (parse_term @"B:num") (SPEC (parse_term @"x:nadd") NADD_CAUCHY)
          ...
          |> THENL <| [MATCH_MP_TAC(LE_IMP DIST_ADDBOUND)
                       |> THEN <| ASM_REWRITE_TAC [MULT_CLAUSES; RIGHT_ADD_DISTRIB; MULT_AC]
          ...
          |> THEN <| GEN_REWRITE_TAC (LAND_CONV << RAND_CONV << RAND_CONV << LAND_CONV) [MULT_SYM]
          |> THEN <| POP_ASSUM(MATCH_MP_TAC << LE_IMP)
          |> THEN <| REWRITE_TAC [LE_EXISTS; RIGHT_ADD_DISTRIB; LEFT_ADD_DISTRIB; MULT_AC]
          |> THEN <| CONV_TAC(ONCE_DEPTH_CONV NUM_CANCEL_CONV)
          |> THEN <| REWRITE_TAC [GSYM EXISTS_REFL])

let NADD_ADDITIVE = 
    prove((parse_term @"!x. ?B. !m n. dist(fn x (m + n),fn x m + fn x n) <= B"), 
          GEN_TAC
          |> THEN <| X_CHOOSE_TAC (parse_term @"B:num") (SPEC (parse_term @"x:nadd") NADD_CAUCHY)
          ...
          |> THENL <| [RULE_ASSUM_TAC(REWRITE_RULE [ADD_EQ_0])
                       |> THEN <| ONCE_REWRITE_TAC [DIST_SYM]
                       |> THEN <| ASM_REWRITE_TAC [ADD_CLAUSES; DIST_LADD_0; LE_ADDR];
                       MATCH_MP_TAC LE_TRANS
          ...
          |> THEN <| GEN_REWRITE_TAC (LAND_CONV << RAND_CONV << LAND_CONV) [RIGHT_ADD_DISTRIB]
          |> THEN <| MATCH_MP_TAC(LE_IMP DIST_ADD2)
          |> THEN <| SUBGOAL_THEN (parse_term @"(m + n) * 3 * B = B * (m + m + n) + B * (n + m + n)") SUBST1_TAC
          |> THENL <| [REWRITE_TAC [SYM(REWRITE_CONV [ARITH] (parse_term @"1 + 1 + 1"))]
                       |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB; LEFT_ADD_DISTRIB; MULT_CLAUSES]
...
let NADD_SUC = 
    prove((parse_term @"!x. ?B. !n. dist(fn x (SUC n),fn x n) <= B"), 
          GEN_TAC
          |> THEN <| X_CHOOSE_TAC (parse_term @"B:num") (SPEC (parse_term @"x:nadd") NADD_ADDITIVE)
...
let NADD_DIST_LEMMA = 
    prove((parse_term @"!x. ?B. !m n. dist(fn x (m + n),fn x m) <= B * n"), 
          GEN_TAC
          |> THEN <| X_CHOOSE_TAC (parse_term @"B:num") (SPEC (parse_term @"x:nadd") NADD_SUC)
          |> THEN <| EXISTS_TAC(parse_term @"B:num")
          |> THEN <| GEN_TAC
          |> THEN <| INDUCT_TAC
...
let NADD_DIST = 
    prove((parse_term @"!x. ?B. !m n. dist(fn x m,fn x n) <= B * dist(m,n)"), 
          GEN_TAC
          |> THEN <| X_CHOOSE_TAC (parse_term @"B:num") (SPEC (parse_term @"x:nadd") NADD_DIST_LEMMA)
          |> THEN <| EXISTS_TAC(parse_term @"B:num")
          |> THEN <| REPEAT GEN_TAC
          |> THEN <| DISJ_CASES_THEN MP_TAC (SPECL [(parse_term @"m:num");
                                                    (parse_term @"n:num")] LE_CASES)
          |> THEN <| DISCH_THEN(CHOOSE_THEN SUBST1_TAC << ONCE_REWRITE_RULE [LE_EXISTS])
          |> THENL <| [ONCE_REWRITE_TAC [DIST_SYM];
                       ALL_TAC]
          |> THEN <| ASM_REWRITE_TAC [DIST_LADD_0])

let NADD_ALTMUL = 
    prove((parse_term @"!x y. ?A B. !n. dist(n * fn x (fn y n),fn x n * fn y n) <= A * n + B"), 
          REPEAT GEN_TAC
          |> THEN <| X_CHOOSE_TAC (parse_term @"B:num") (SPEC (parse_term @"x:nadd") NADD_CAUCHY)
          |> THEN <| MP_TAC(SPEC (parse_term @"y:nadd") NADD_BOUND)
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"M:num") (X_CHOOSE_TAC(parse_term @"L:num")))
          |> THEN <| MAP_EVERY EXISTS_TAC [(parse_term @"B * (1 + M)");
                                           (parse_term @"B * L")]
          |> THEN <| GEN_TAC
          |> THEN <| GEN_REWRITE_TAC (LAND_CONV << RAND_CONV << RAND_CONV) [MULT_SYM]
          |> THEN <| MATCH_MP_TAC LE_TRANS
...
let NADD_EQ_SYM = 
    prove((parse_term @"!x y. x === y <=> y === x"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [nadd_eq]
          |> THEN <| GEN_REWRITE_TAC (RAND_CONV << ONCE_DEPTH_CONV) [DIST_SYM]
          |> THEN <| REFL_TAC)

let NADD_EQ_TRANS = 
    prove((parse_term @"!x y z. x === y /\ y === z ==> x === z"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [nadd_eq]
          |> THEN 
          <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"B1:num")) (X_CHOOSE_TAC(parse_term @"B2:num")))
...
let NADD_LE_WELLDEF_LEMMA = 
    prove((parse_term @"!x x' y y'. x === x' /\ y === y' /\ x <<= y ==> x' <<= y'"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [nadd_eq; nadd_le]
          |> THEN <| REWRITE_TAC [DIST_LE_CASES; FORALL_AND_THM]
          |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"B1:num")) MP_TAC)
          |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"B2:num")) MP_TAC)
...
let NADD_LE_ANTISYM = 
    prove((parse_term @"!x y. x <<= y /\ y <<= x <=> (x === y)"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [nadd_le; nadd_eq; DIST_LE_CASES]
          |> THEN <| EQ_TAC
          |> THENL <| [DISCH_THEN
                           (CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"B1:num")) (X_CHOOSE_TAC(parse_term @"B2:num")))
                       |> THEN <| EXISTS_TAC(parse_term @"B1 + B2")
                       |> THEN <| GEN_TAC
                       |> THEN <| CONJ_TAC
                       |> THEN <| FIRST_ASSUM(MATCH_MP_TAC << LE_IMP)
                       |> THEN <| ASM_REWRITE_TAC [ADD_ASSOC; LE_ADD_RCANCEL; LE_ADD; LE_ADDR];
...
let NADD_LE_TOTAL_LEMMA = 
    prove((parse_term @"!x y. ~(x <<= y) ==> !B. ?n. ~(n = 0) /\ fn y n + B < fn x n"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [nadd_le; NOT_FORALL_THM; NOT_EXISTS_THM]
          |> THEN <| REWRITE_TAC [NOT_LE]
          |> THEN <| DISCH_TAC
          |> THEN <| GEN_TAC
          |> THEN <| POP_ASSUM(X_CHOOSE_TAC(parse_term @"n:num") << SPEC(parse_term @"B + fn x 0"))
          |> THEN <| EXISTS_TAC(parse_term @"n:num")
          |> THEN <| POP_ASSUM MP_TAC
          |> THEN <| ASM_CASES_TAC(parse_term @"n = 0")
...
let NADD_LE_TOTAL = 
    prove((parse_term @"!x y. x <<= y \/ y <<= x"), 
          REPEAT GEN_TAC
          |> THEN <| GEN_REWRITE_TAC I [TAUT(parse_term @"a <=> ~ ~ a")]
          |> THEN <| X_CHOOSE_TAC (parse_term @"B1:num") (SPEC (parse_term @"x:nadd") NADD_CAUCHY)
          |> THEN <| X_CHOOSE_TAC (parse_term @"B2:num") (SPEC (parse_term @"y:nadd") NADD_CAUCHY)
          |> THEN <| PURE_ONCE_REWRITE_TAC [DE_MORGAN_THM]
          |> THEN <| DISCH_THEN(MP_TAC << end_itlist CONJ << map(MATCH_MP NADD_LE_TOTAL_LEMMA) << CONJUNCTS)
          |> THEN <| REWRITE_TAC [AND_FORALL_THM]
          |> THEN <| DISCH_THEN(MP_TAC << SPEC(parse_term @"B1 + B2"))
          |> THEN <| REWRITE_TAC [RIGHT_AND_EXISTS_THM]
          |> THEN <| REWRITE_TAC [LEFT_AND_EXISTS_THM]
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"m:num") (X_CHOOSE_THEN (parse_term @"n:num") MP_TAC))
          |> THEN <| DISCH_THEN(MP_TAC << MATCH_MP(ITAUT(parse_term @"(~a /\ b) /\ (~c /\ d) ==> ~(c \/ ~b) /\ ~(a \/ ~d)")))
          |> THEN <| REWRITE_TAC [NOT_LT;
                                  GSYM LE_MULT_LCANCEL]
          |> THEN <| REWRITE_TAC [NOT_LE]
          |> THEN <| DISCH_THEN(MP_TAC << MATCH_MP LT_ADD2)
          |> THEN <| REWRITE_TAC [NOT_LT]
          |> THEN <| REWRITE_TAC [LEFT_ADD_DISTRIB]
          |> THEN <| ONCE_REWRITE_TAC [AC ADD_AC (parse_term @"(a + b + c) + (d + e + f) = (d + b + e) + (a + c + f)")]
          |> THEN <| MATCH_MP_TAC LE_ADD2
          |> THEN <| REWRITE_TAC [GSYM RIGHT_ADD_DISTRIB]
          |> THEN <| CONJ_TAC
          |> THEN <| GEN_REWRITE_TAC (RAND_CONV << RAND_CONV) [MULT_SYM]
...
let NADD_ADD = 
    prove((parse_term @"!x y. fn(x ++ y) = \n. fn x n + fn y n"), 
          ...
          |> THEN <| MAP_EVERY X_GEN_TAC [(parse_term @"m:num");
                                          (parse_term @"n:num")]
          |> THEN <| GEN_REWRITE_TAC (LAND_CONV << ONCE_DEPTH_CONV) [LEFT_ADD_DISTRIB]
          |> THEN <| MATCH_MP_TAC(LE_IMP DIST_ADD2)
          |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB]
          |> THEN <| MATCH_MP_TAC LE_ADD2
          |> THEN <| ASM_REWRITE_TAC [])
...
let NADD_ADD_WELLDEF = 
    prove((parse_term @"!x x' y y'. x === x' /\ y === y' ==> (x ++ y === x' ++ y')"), 
          ...
          |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"B1:num")) (X_CHOOSE_TAC(parse_term @"B2:num")))
          |> THEN <| EXISTS_TAC(parse_term @"B1 + B2")
          |> THEN <| X_GEN_TAC(parse_term @"n:num")
          |> THEN <| MATCH_MP_TAC(LE_IMP DIST_ADD2)
          |> THEN <| MATCH_MP_TAC LE_ADD2
...
let NADD_LE_EXISTS = 
    prove((parse_term @"!x y. x <<= y ==> ?d. y === x ++ d"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [nadd_le]
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"B:num") MP_TAC)
          |> THEN <| REWRITE_TAC [LE_EXISTS; SKOLEM_THM]
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"d:num->num") (ASSUME_TAC << GSYM))
          ...
          |> THENL <| [REWRITE_TAC [GSYM nadd_rep;
                       ...
                       |> THEN <| MAP_EVERY EXISTS_TAC [(parse_term @"m * fn x n");
                                                        (parse_term @"n * fn x m")]
                       ...
                       |> THEN <| ONCE_REWRITE_TAC [ADD_SYM]
                       |> THEN <| ASM_REWRITE_TAC []
                       |> THEN <| GEN_REWRITE_TAC (LAND_CONV << ONCE_DEPTH_CONV) [MULT_SYM]
...
let NADD_MUL = 
    prove((parse_term @"!x y. fn(x ** y) = \n. fn x (fn y n)"), 
          ...
          |> THEN <| REWRITE_TAC [nadd_mul;
                                  GSYM nadd_rep;
                                  is_nadd]
          |> THEN <| X_CHOOSE_TAC (parse_term @"B:num") (SPEC (parse_term @"y:nadd") NADD_CAUCHY)
          |> THEN <| X_CHOOSE_TAC (parse_term @"C:num") (SPEC (parse_term @"x:nadd") NADD_DIST)
          |> THEN <| X_CHOOSE_TAC (parse_term @"D:num") (SPEC (parse_term @"x:nadd") NADD_MULTIPLICATIVE)
          ...
          |> THEN <| REPEAT GEN_TAC
          |> THEN <| MATCH_MP_TAC LE_TRANS
          |> THEN <| EXISTS_TAC(parse_term @"(D * m + D) + (D * n + D) + C * B * (m + n)")
          |> THEN <| CONJ_TAC
          |> THENL <| [MATCH_MP_TAC(LE_IMP DIST_TRIANGLE)
                       ...
                       |> THEN <| REWRITE_TAC [LEFT_ADD_DISTRIB; RIGHT_ADD_DISTRIB; MULT_ASSOC; ADD_AC]])
...
let NADD_MUL_SYM = 
    prove((parse_term @"!x y. (x ** y) === (y ** x)"), 
          ...
          |> THEN <| X_CHOOSE_THEN (parse_term @"A1:num") MP_TAC (SPECL [(parse_term @"x:nadd");
                                                                         (parse_term @"y:nadd")] NADD_ALTMUL)
          |> THEN <| DISCH_THEN(X_CHOOSE_TAC(parse_term @"B1:num"))
          |> THEN <| X_CHOOSE_THEN (parse_term @"A2:num") MP_TAC (SPECL [(parse_term @"y:nadd");
          ...
          |> THEN <| GEN_TAC
          |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB]
          |> THEN <| ONCE_REWRITE_TAC [AC ADD_AC (parse_term @"(a + b) + (c + d) = (a + c) + (b + d)")]
          |> THEN <| MATCH_MP_TAC(LE_IMP DIST_TRIANGLE)
...
let NADD_EQ_IMP_LE = 
    prove((parse_term @"!x y. x === y ==> x <<= y"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [nadd_eq; nadd_le; DIST_LE_CASES]
          |> THEN <| DISCH_THEN(X_CHOOSE_TAC(parse_term @"B:num"))
          |> THEN <| EXISTS_TAC(parse_term @"B:num")
          |> THEN <| ASM_REWRITE_TAC [])

let NADD_LE_LMUL = 
    prove((parse_term @"!x y z. y <<= z ==> (x ** y) <<= (x ** z)"), 
          REPEAT GEN_TAC
          |> THEN <| DISCH_THEN(X_CHOOSE_TAC(parse_term @"d:nadd") << MATCH_MP NADD_LE_EXISTS)
...
let NADD_LE_RADD = 
    prove((parse_term @"!x y z. x ++ z <<= y ++ z <=> x <<= y"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [nadd_le; NADD_ADD]
          |> THEN <| GEN_REWRITE_TAC (LAND_CONV << funpow 2 BINDER_CONV << RAND_CONV) [ADD_SYM]
          |> THEN <| REWRITE_TAC [ADD_ASSOC; LE_ADD_RCANCEL]
          |> THEN <| GEN_REWRITE_TAC (LAND_CONV << funpow 2 BINDER_CONV << RAND_CONV) [ADD_SYM]
          |> THEN <| REFL_TAC)
...
let NADD_ARCH_MULT = 
    prove((parse_term @"!x k. ~(x === &0) ==> ?N. &k <<= &N ** x"), 
          REPEAT GEN_TAC
          ...
          |> THEN <| MATCH_MP_TAC(GEN_ALL(fst(EQ_IMP_RULE(SPEC_ALL LE_ADD_RCANCEL))))
...
let NADD_ARCH_ZERO = 
    prove((parse_term @"!x k. (!n. &n ** x <<= k) ==> (x === &0)"), 
          REPEAT GEN_TAC
          |> THEN <| CONV_TAC CONTRAPOS_CONV
          |> THEN <| DISCH_TAC
          |> THEN <| REWRITE_TAC [NOT_FORALL_THM]
          |> THEN <| X_CHOOSE_TAC (parse_term @"p:num") (SPEC (parse_term @"k:nadd") NADD_ARCH)
          |> THEN <| FIRST_ASSUM(MP_TAC << MATCH_MP NADD_ARCH_MULT)
          |> THEN <| DISCH_THEN(X_CHOOSE_TAC(parse_term @"N:num") << SPEC(parse_term @"p:num"))
          |> THEN <| EXISTS_TAC(parse_term @"N + 1")
          |> THEN <| DISCH_TAC
          |> THEN <| UNDISCH_TAC(parse_term @"~(x === &0)")
          |> THEN <| REWRITE_TAC [GSYM NADD_LE_ANTISYM;
                                  NADD_LE_0]
          |> THEN <| MATCH_MP_TAC(GEN_ALL(fst(EQ_IMP_RULE(SPEC_ALL NADD_LE_RADD))))
          |> THEN <| EXISTS_TAC(parse_term @"&N ** x")
          |> THEN <| MATCH_MP_TAC NADD_LE_TRANS
          |> THEN <| EXISTS_TAC(parse_term @"k:nadd")
          |> THEN <| CONJ_TAC
          |> THENL <| [SUBGOAL_THEN (parse_term @"&(N + 1) ** x === x ++ &N ** x") MP_TAC
                       |> THENL <| [ONCE_REWRITE_TAC [ADD_SYM]
                                    |> THEN <| MATCH_MP_TAC NADD_EQ_TRANS
                                    |> THEN <| EXISTS_TAC(parse_term @"&1 ** x ++ &N ** x")
                                    |> THEN <| CONJ_TAC
                                    |> THENL <| [MATCH_MP_TAC NADD_EQ_TRANS
                                                 |> THEN <| EXISTS_TAC(parse_term @"(&1 ++ &N) ** x")
                                                 |> THEN <| CONJ_TAC
                                                 |> THENL 
                                                 <| [MESON_TAC 
                                                         [NADD_OF_NUM_ADD; NADD_MUL_WELLDEF; NADD_EQ_REFL; NADD_EQ_SYM];
                       ...
                       ASM_MESON_TAC [NADD_LE_TRANS; NADD_LE_WELLDEF; NADD_EQ_REFL; NADD_ADD_LID]])

let NADD_ARCH_LEMMA = 
    prove((parse_term @"!x y z. (!n. &n ** x <<= &n ** y ++ z) ==> x <<= y"), 
          REPEAT STRIP_TAC
          |> THEN <| DISJ_CASES_TAC(SPECL [(parse_term @"x:nadd");
                                           (parse_term @"y:nadd")] NADD_LE_TOTAL)
          |> THEN <| ASM_REWRITE_TAC []
          |> THEN <| FIRST_ASSUM(X_CHOOSE_TAC(parse_term @"d:nadd") << MATCH_MP NADD_LE_EXISTS)
          |> THEN <| MATCH_MP_TAC NADD_EQ_IMP_LE
          |> THEN <| MATCH_MP_TAC NADD_EQ_TRANS
          ...
          |> THEN <| CONJ_TAC
          |> THENL <| [MATCH_MP_TAC NADD_ADD_WELLDEF
                       |> THEN <| REWRITE_TAC [NADD_EQ_REFL]
                       |> THEN <| MATCH_MP_TAC NADD_ARCH_ZERO
                       |> THEN <| EXISTS_TAC(parse_term @"z:nadd")
                       |> THEN 
                       <| ASM_MESON_TAC [NADD_MUL_WELLDEF; NADD_LE_WELLDEF; NADD_LDISTRIB; NADD_LE_LADD; NADD_EQ_REFL];
                       ASM_MESON_TAC [NADD_ADD_LID; NADD_ADD_WELLDEF; NADD_EQ_TRANS; NADD_ADD_SYM]])
...
let NADD_COMPLETE = 
    prove((parse_term @"!P. (?x. P x) /\ (?M. !x. P x ==> x <<= M) ==>
       ?M. (!x. P x ==> x <<= M) /\
          ...
          GEN_TAC
          |> THEN 
          <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"a:nadd")) (X_CHOOSE_TAC(parse_term @"m:nadd")))
          |> THEN <| SUBGOAL_THEN (parse_term @"!n. ?r. (?x. P x /\ &r <<= &n ** x) /\
             !r'. (?x. P x /\ &r' <<= &n ** x) ==> r' <= r") MP_TAC
          |> THENL <| [GEN_TAC
                       |> THEN <| REWRITE_TAC [GSYM num_MAX]
                       |> THEN <| CONJ_TAC
                       |> THENL <| [MAP_EVERY EXISTS_TAC [(parse_term @"0");
                                                          (parse_term @"a:nadd")]
                                    |> THEN <| ASM_REWRITE_TAC [NADD_LE_0];
                                    X_CHOOSE_TAC (parse_term @"N:num") (SPEC (parse_term @"m:nadd") NADD_ARCH)
                                    |> THEN <| EXISTS_TAC(parse_term @"n * N")
                                    |> THEN <| X_GEN_TAC(parse_term @"p:num")
                                    |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"w:nadd") STRIP_ASSUME_TAC)
                                    ...
                                    |> THEN <| EXISTS_TAC(parse_term @"&n ** &N")
                                    |> THEN <| CONJ_TAC
                                    |> THENL <| [MATCH_MP_TAC NADD_LE_LMUL
                                                 |> THEN <| MATCH_MP_TAC NADD_LE_TRANS
                       ...
                       |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"r:num->num") (fun th -> 
                                                 let th1, th2 = CONJ_PAIR(SPEC (parse_term @"n:num") th)
                                                 MAP_EVERY (MP_TAC << GEN(parse_term @"n:num")) [th1; th2]))
                       |> THEN 
                       <| DISCH_THEN(MP_TAC << GEN(parse_term @"n:num") << SPECL [(parse_term @"n:num");
                                                                                  (parse_term @"SUC(r(n:num))")])
                       |> THEN <| REWRITE_TAC [LE_SUC_LT; LT_REFL; NOT_EXISTS_THM]
                       |> THEN 
                       <| DISCH_THEN
                              (ASSUME_TAC << GENL [(parse_term @"n:num");
                                                   (parse_term @"x:nadd")] 
                               << MATCH_MP(ITAUT(parse_term @"(a \/ b) /\ ~(c /\ b) ==> c ==> a")) 
                               << CONJ(SPECL [(parse_term @"&n ** x");
                                              (parse_term @"&(SUC(r(n:num)))")] NADD_LE_TOTAL) << SPEC_ALL)
                       |> THEN <| DISCH_TAC]
          |> THEN <| SUBGOAL_THEN (parse_term @"!n i. i * r(n) <= n * r(i) + n") ASSUME_TAC
          |> THENL <| [REPEAT GEN_TAC
                       |> THEN 
                       <| FIRST_ASSUM
                              (X_CHOOSE_THEN (parse_term @"x:nadd") STRIP_ASSUME_TAC << SPEC(parse_term @"n:num"))
                       |> THEN <| ONCE_REWRITE_TAC [GSYM NADD_OF_NUM_LE]
                       |> THEN <| MATCH_MP_TAC NADD_LE_TRANS
                       ...
                       |> THENL <| [MATCH_MP_TAC NADD_LE_TRANS
                                    ...
                                    |> THEN <| EXISTS_TAC(parse_term @"&n ** &(SUC(r(i:num)))")
                                    |> THEN <| CONJ_TAC
                                    |> THENL <| [MATCH_MP_TAC NADD_LE_TRANS
                                                 |> THEN <| EXISTS_TAC(parse_term @"&n ** &i ** x")
                                                 |> THEN <| CONJ_TAC
                                                 |> THENL <| [MATCH_MP_TAC NADD_EQ_IMP_LE
                                                              |> THEN <| MATCH_MP_TAC NADD_EQ_TRANS
                                                              |> THEN <| EXISTS_TAC(parse_term @"(&i ** &n) ** x")
                                                              |> THEN <| REWRITE_TAC [NADD_MUL_ASSOC]
                                                              |> THEN <| MATCH_MP_TAC NADD_EQ_TRANS
                                                              |> THEN <| EXISTS_TAC(parse_term @"(&n ** &i) ** x")
                                                              |> THEN 
                                                              <| REWRITE_TAC 
                                                                     [ONCE_REWRITE_RULE [NADD_EQ_SYM] NADD_MUL_ASSOC]
                                                              |> THEN <| MATCH_MP_TAC NADD_MUL_WELLDEF
                                                              |> THEN <| REWRITE_TAC [NADD_MUL_SYM; NADD_EQ_REFL];
                                                              MATCH_MP_TAC NADD_LE_LMUL
                                                              |> THEN <| FIRST_ASSUM MATCH_MP_TAC
                                                              |> THEN <| ASM_REWRITE_TAC []];
          ...
          |> THENL <| [REWRITE_TAC [GSYM nadd_rep]
                       ...
                       |> THEN <| GEN_REWRITE_TAC (LAND_CONV << funpow 2 BINDER_CONV << funpow 2 RAND_CONV) [ADD_SYM]
                       |> THEN <| REWRITE_TAC []
          ...
          |> THENL <| [X_GEN_TAC(parse_term @"x:nadd")
                       ...
                       |> THENL <| [FIRST_ASSUM MATCH_MP_TAC
                                    |> THEN <| ASM_REWRITE_TAC [];
                                    ASM_REWRITE_TAC [nadd_le; NADD_ADD; NADD_MUL; NADD_OF_NUM]
                                    |> THEN <| ONCE_REWRITE_TAC [ADD_SYM]
                                    |> THEN <| REWRITE_TAC [ADD1; RIGHT_ADD_DISTRIB]
                                    |> THEN <| REWRITE_TAC [MULT_2; MULT_CLAUSES; ADD_ASSOC; LE_ADD_RCANCEL]
                                    |> THEN <| REWRITE_TAC [GSYM ADD_ASSOC]
                                    |> THEN <| ONCE_REWRITE_TAC [ADD_SYM]
                                    ...
                                    |> THEN <| MATCH_MP_TAC LE_TRANS
                                    |> THEN <| EXISTS_TAC(parse_term @"n * r(i:num) + n")
                                    |> THEN <| ASM_REWRITE_TAC [LE_ADD_LCANCEL; ADD_CLAUSES]];
                       ...
                       |> THEN <| CONJ_TAC
                       |> THENL <| [ASM_REWRITE_TAC [nadd_le; NADD_ADD; NADD_MUL; NADD_OF_NUM]
                                    |> THEN <| EXISTS_TAC(parse_term @"0")
                                    |> THEN <| REWRITE_TAC [ADD_CLAUSES; MULT_CLAUSES]
                                    |> THEN <| GEN_TAC
                                    |> THEN <| GEN_REWRITE_TAC (RAND_CONV << LAND_CONV) [MULT_SYM]
                                    |> THEN <| ASM_REWRITE_TAC [];
                                    REWRITE_TAC [NADD_LE_RADD]
                                    |> THEN 
                                    <| FIRST_ASSUM
                                           (X_CHOOSE_THEN (parse_term @"x:nadd") MP_TAC << SPEC(parse_term @"n:num"))
...
let NADD_UBOUND = 
    prove((parse_term @"!x. ?B N. !n. N <= n ==> fn x n <= B * n"), 
          GEN_TAC
          |> THEN <| X_CHOOSE_THEN (parse_term @"A1:num") (X_CHOOSE_TAC(parse_term @"A2:num")) 
          ...
          |> THEN <| ASM_REWRITE_TAC []
          |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB; LE_ADD_LCANCEL]
          |> THEN <| GEN_REWRITE_TAC LAND_CONV [GSYM(el 3 (CONJUNCTS MULT_CLAUSES))]
          |> THEN <| ASM_REWRITE_TAC [LE_MULT_LCANCEL])

let NADD_NONZERO = 
    prove((parse_term @"!x. ~(x === &0) ==> ?N. !n. N <= n ==> ~(fn x n = 0)"), 
          GEN_TAC
          ...
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"A1:num") (X_CHOOSE_TAC(parse_term @"A2:num")))
...
let NADD_LBOUND = 
    prove((parse_term @"!x. ~(x === &0) ==> ?A N. !n. N <= n ==> n <= A * fn x n"), 
          GEN_TAC
          |> THEN <| DISCH_TAC
          |> THEN <| FIRST_ASSUM(X_CHOOSE_TAC(parse_term @"N:num") << MATCH_MP NADD_NONZERO)
          |> THEN <| FIRST_ASSUM(MP_TAC << MATCH_MP NADD_ARCH_MULT)
          |> THEN <| DISCH_THEN(MP_TAC << SPEC(parse_term @"1"))
          |> THEN <| REWRITE_TAC [nadd_le; NADD_MUL; NADD_OF_NUM; MULT_CLAUSES]
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"A1:num") (X_CHOOSE_TAC(parse_term @"A2:num")))
          ...
          |> THEN <| DISJ2_TAC
          |> THEN <| REWRITE_TAC [GSYM(REWRITE_CONV [ARITH_SUC] (parse_term @"SUC 0"))]
...
let NADD_MUL_LINV_LEMMA0 = 
    prove((parse_term @"!x. ~(x === &0) ==> ?A B. !n. nadd_rinv x n <= A * n + B"), 
          GEN_TAC
          |> THEN <| DISCH_TAC
          |> THEN <| ONCE_REWRITE_TAC [BOUNDS_IGNORE]
          |> THEN <| FIRST_ASSUM(MP_TAC << MATCH_MP NADD_LBOUND)
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"A:num") (X_CHOOSE_TAC(parse_term @"N:num")))
          ...
          |> THENL <| [DISJ2_TAC
                       ...
                       |> THEN <| DISCH_THEN(fun t -> GEN_REWRITE_TAC RAND_CONV [CONJUNCT1(SPEC_ALL t)])
                       |> THEN <| GEN_REWRITE_TAC LAND_CONV [MULT_SYM]
                       |> THEN <| REWRITE_TAC [LE_ADD]])

let NADD_MUL_LINV_LEMMA1 = 
    prove((parse_term @"!x n. ~(fn x n = 0) ==> dist(fn x n * nadd_rinv(x) n, n * n) <= fn x n"), 
          REPEAT GEN_TAC
          |> THEN <| DISCH_THEN(MP_TAC << MATCH_MP DIVISION)
          |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 SUBST1_TAC ASSUME_TAC << SPEC(parse_term @"n * n"))
          |> THEN <| REWRITE_TAC [nadd_rinv]
          |> THEN <| GEN_REWRITE_TAC (LAND_CONV << RAND_CONV << LAND_CONV) [MULT_SYM]
...
let NADD_MUL_LINV_LEMMA4 = 
    prove((parse_term @"!x. ~(x === &0) ==> ?N. !m n. N <= m /\ N <= n ==>
        (fn x m * fn x n) * dist(m * nadd_rinv(x) n,n * nadd_rinv(x) m) <=
          ...
          |> THEN <| CONJ_TAC
          |> THENL <| [ANTE_RES_THEN (MP_TAC << SPEC(parse_term @"m:num")) (ASSUME(parse_term @"N <= n"));
                       ANTE_RES_THEN (MP_TAC << SPEC(parse_term @"n:num")) (ASSUME(parse_term @"N <= m"))]
...
let NADD_MUL_LINV_LEMMA5 = 
    prove((parse_term @"!x. ~(x === &0) ==> ?B N. !m n. N <= m /\ N <= n ==>
        ...
        B * (m * n) * (m + n)"),
          ...
          |> THEN <| X_CHOOSE_TAC (parse_term @"B1:num") (SPEC (parse_term @"x:nadd") NADD_CAUCHY)
          |> THEN 
          <| X_CHOOSE_THEN (parse_term @"B2:num") (X_CHOOSE_TAC(parse_term @"N2:num")) 
          ...
          |> THENL <| [GEN_REWRITE_TAC RAND_CONV [MULT_SYM]
                       ...
                       |> THEN <| ASM_REWRITE_TAC [LE_MULT_LCANCEL];
                       ONCE_REWRITE_TAC [AC MULT_AC (parse_term @"(a * b) * (c * d) * e = ((a * c) * (b * d)) * e")]
                       |> THEN <| REWRITE_TAC [LE_MULT_RCANCEL]
                       |> THEN <| DISJ1_TAC
...
let NADD_MUL_LINV_LEMMA6 = 
    prove((parse_term @"!x. ~(x === &0) ==> ?B N. !m n. N <= m /\ N <= n ==>
        ...
        B * (m * n) * (m + n)"),
          ...
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"B1:num") (X_CHOOSE_TAC(parse_term @"N1:num")))
          |> THEN <| FIRST_ASSUM(MP_TAC << MATCH_MP NADD_LBOUND)
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"B2:num") (X_CHOOSE_TAC(parse_term @"N2:num")))
          |> THEN <| EXISTS_TAC(parse_term @"B1 * B2 * B2")
          |> THEN <| EXISTS_TAC(parse_term @"N1 + N2")
          |> THEN <| REPEAT STRIP_TAC
          |> THEN <| MATCH_MP_TAC LE_TRANS
          ...
          |> THENL <| [REWRITE_TAC [MULT_ASSOC; LE_MULT_RCANCEL]
                       |> THEN <| DISJ1_TAC
                       |> THEN <| ONCE_REWRITE_TAC [AC MULT_AC (parse_term @"((a * b) * c) * d = (a * c) * (b * d)")]
                       |> THEN <| MATCH_MP_TAC LE_MULT2
                       |> THEN <| CONJ_TAC
                       |> THEN <| FIRST_ASSUM MATCH_MP_TAC;
                       ONCE_REWRITE_TAC 
                           [AC MULT_AC (parse_term @"(a * b * c) * (d * e) * f = (b * c) * (a * (d * e) * f)")]
                       |> THEN <| REWRITE_TAC [LE_MULT_LCANCEL]
                       |> THEN <| DISJ2_TAC
                       |> THEN <| FIRST_ASSUM MATCH_MP_TAC
                       |> THEN <| CONJ_TAC]
...
let NADD_MUL_LINV_LEMMA7 = 
    prove((parse_term @"!x. ~(x === &0) ==> ?B N. !m n. N <= m /\ N <= n ==>
        dist(m * nadd_rinv(x) n,n * nadd_rinv(x) m) <= B * (m + n)"),
          ...
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"B:num") (X_CHOOSE_TAC(parse_term @"N:num")))
          ...
          |> THENL <| [CONJ_TAC
                       ...
                       |> THEN <| ONCE_REWRITE_TAC [AC MULT_AC (parse_term @"a * b * c = b * a * c")]
                       |> THEN <| REWRITE_TAC [LE_MULT_LCANCEL]
                       ...
                       |> THEN <| REWRITE_TAC [NOT_LE;
                                               GSYM LE_SUC_LT]
                       |> THEN <| REWRITE_TAC [EQT_ELIM(REWRITE_CONV [ARITH] (parse_term @"SUC 0 = 1 * 1"))]
                       |> THEN <| MATCH_MP_TAC LE_MULT2
                       |> THEN <| CONJ_TAC
                       |> THEN <| MATCH_MP_TAC LE_TRANS
                       |> THEN <| EXISTS_TAC(parse_term @"N + 1")
...
let NADD_MUL_LINV_LEMMA7a = 
    prove((parse_term @"!x. ~(x === &0) ==> !N. ?A B. !m n. m <= N ==>
        dist(m * nadd_rinv(x) n,n * nadd_rinv(x) m) <= A * n + B"),
          GEN_TAC
          |> THEN <| DISCH_THEN(MP_TAC << MATCH_MP NADD_MUL_LINV_LEMMA0)
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"A0:num") (X_CHOOSE_TAC(parse_term @"B0:num")))
          |> THEN <| INDUCT_TAC
          |> THENL <| [MAP_EVERY EXISTS_TAC [(parse_term @"nadd_rinv x 0");
                       ...
                       |> THEN <| REWRITE_TAC [LE]
                       |> THEN <| DISCH_THEN SUBST1_TAC
                       |> THEN <| REWRITE_TAC [MULT_CLAUSES; DIST_LZERO; ADD_CLAUSES]
                       |> THEN <| GEN_REWRITE_TAC RAND_CONV [MULT_SYM]
                       |> THEN <| MATCH_ACCEPT_TAC LE_REFL;
                       FIRST_ASSUM(X_CHOOSE_THEN (parse_term @"A:num") (X_CHOOSE_TAC(parse_term @"B:num")))
                       |> THEN <| EXISTS_TAC(parse_term @"A + (nadd_rinv(x)(SUC N) + SUC N * A0)")
                       |> THEN <| EXISTS_TAC(parse_term @"SUC N * B0 + B")
                       |> THEN <| REPEAT GEN_TAC
                       |> THEN <| REWRITE_TAC [LE]
                       |> THEN <| DISCH_THEN(DISJ_CASES_THEN2 SUBST1_TAC ASSUME_TAC)
                       |> THENL <| [MATCH_MP_TAC LE_TRANS
                                    |> THEN <| EXISTS_TAC(parse_term @"SUC N * nadd_rinv x n + n * nadd_rinv x (SUC N)")
                                    |> THEN <| REWRITE_TAC [DIST_ADDBOUND]
                                    |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB]
                                    |> THEN 
                                    <| ONCE_REWRITE_TAC 
                                           [AC ADD_AC (parse_term @"(a + b + c) + d + e = (c + d) + (b + a + e)")]
                                    |> THEN <| MATCH_MP_TAC LE_ADD2
                                    |> THEN <| CONJ_TAC
                                    |> THENL <| [REWRITE_TAC [GSYM MULT_ASSOC;
                                                              GSYM LEFT_ADD_DISTRIB]
                                                 |> THEN <| ASM_REWRITE_TAC [LE_MULT_LCANCEL];
                                                 GEN_REWRITE_TAC LAND_CONV [MULT_SYM]
                                    ...
                                    |> THENL <| [FIRST_ASSUM MATCH_MP_TAC
                                                 |> THEN <| ASM_REWRITE_TAC [];
                                                 REWRITE_TAC [ADD_ASSOC; LE_ADD_RCANCEL]
                                                 |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB;
                                                                         GSYM ADD_ASSOC;
                                                                         LE_ADD]]]])
...
let NADD_MUL_LINV_LEMMA8 = 
    prove((parse_term @"!x. ~(x === &0) ==>
        ?B. !m n. dist(m * nadd_rinv(x) n,n * nadd_rinv(x) m) <= B * (m + n)"),
          ...
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"B0:num") (X_CHOOSE_TAC(parse_term @"N:num")))
          |> THEN <| FIRST_ASSUM(MP_TAC << SPEC(parse_term @"N:num") << MATCH_MP NADD_MUL_LINV_LEMMA7a)
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"A:num") (X_CHOOSE_TAC(parse_term @"B:num")))
          |> THEN <| MATCH_MP_TAC BOUNDS_NOTZERO
          |> THEN <| REWRITE_TAC [DIST_REFL]
          |> THEN <| EXISTS_TAC(parse_term @"A + B0")
          |> THEN <| EXISTS_TAC(parse_term @"B:num")
          |> THEN <| REPEAT GEN_TAC
          |> THEN <| DISJ_CASES_THEN2 ASSUME_TAC MP_TAC (SPECL [(parse_term @"N:num");
                                                                (parse_term @"m:num")] LE_CASES)
          |> THENL <| [DISJ_CASES_THEN2 ASSUME_TAC MP_TAC (SPECL [(parse_term @"N:num");
                                                                  (parse_term @"n:num")] LE_CASES)
                       |> THENL <| [MATCH_MP_TAC LE_TRANS
                                    |> THEN <| EXISTS_TAC(parse_term @"B0 * (m + n)")
                                    |> THEN <| CONJ_TAC
                                    |> THENL <| [FIRST_ASSUM MATCH_MP_TAC
                                                 |> THEN <| ASM_REWRITE_TAC [];
                                                 GEN_REWRITE_TAC (RAND_CONV << funpow 2 LAND_CONV) [ADD_SYM]
                                                 |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB;
                                                                         GSYM ADD_ASSOC;
                                                                         LE_ADD]];
                       ...
                       |> THEN <| EXISTS_TAC(parse_term @"A * n + B")
                       |> THEN <| ASM_REWRITE_TAC [LE_ADD_RCANCEL]
                       |> THEN <| GEN_REWRITE_TAC (RAND_CONV << RAND_CONV) [ADD_SYM]
                       |> THEN <| REWRITE_TAC [LEFT_ADD_DISTRIB;
                                               RIGHT_ADD_DISTRIB;
                                               GSYM ADD_ASSOC;
                                               LE_ADD]])
...
let NADD_MUL_LINV = 
    prove((parse_term @"!x. ~(x === &0) ==> inv(x) ** x === &1"), 
          GEN_TAC
          |> THEN <| DISCH_TAC
          |> THEN <| REWRITE_TAC [nadd_eq; NADD_MUL]
          |> THEN <| ONCE_REWRITE_TAC [BOUNDS_DIVIDED]
          |> THEN <| X_CHOOSE_THEN (parse_term @"A1:num") (X_CHOOSE_TAC(parse_term @"B1:num")) 
                 (SPECL [(parse_term @"inv(x)");
                         (parse_term @"x:nadd")] NADD_ALTMUL)
          |> THEN <| REWRITE_TAC [DIST_LMUL; NADD_OF_NUM; MULT_CLAUSES]
          |> THEN <| FIRST_ASSUM(X_CHOOSE_TAC(parse_term @"N:num") << MATCH_MP NADD_MUL_LINV_LEMMA2)
          |> THEN 
          <| X_CHOOSE_THEN (parse_term @"A':num") (X_CHOOSE_TAC(parse_term @"B':num")) 
                 (SPEC (parse_term @"x:nadd") NADD_BOUND)
          |> THEN <| SUBGOAL_THEN (parse_term @"?A2 B2. !n. dist(fn x n * nadd_rinv x n,n * n) <= A2 * n + B2") STRIP_ASSUME_TAC
    ...
    prove
        ((parse_term @"!x y. x === y ==> inv(x) === inv(y)"), 
         ...
         |> THENL <| [SUBGOAL_THEN (parse_term @"y === &0") ASSUME_TAC
                      ...
                      |> THENL <| [ASM_MESON_TAC [NADD_EQ_TRANS; NADD_EQ_SYM];
                                   ALL_TAC]]
         |> THEN <| TAC (parse_term @"inv(y) ** &1") [NADD_MUL_SYM; NADD_MUL_LID; NADD_EQ_TRANS]
         |> THEN <| TAC (parse_term @"inv(y) ** (inv(x) ** x)") [NADD_MUL_LINV; NADD_MUL_WELLDEF; NADD_EQ_REFL]
         |> THEN <| TAC (parse_term @"inv(y) ** (inv(x) ** y)") [NADD_MUL_WELLDEF; NADD_EQ_REFL; NADD_EQ_SYM]
         |> THEN  <| TAC (parse_term @"(inv(y) ** y) ** inv(x)") 
                        [NADD_MUL_ASSOC; NADD_MUL_SYM; NADD_EQ_TRANS; NADD_MUL_WELLDEF; NADD_EQ_REFL]
         |> THEN <| ASM_MESON_TAC [NADD_MUL_LINV; NADD_MUL_WELLDEF; NADD_EQ_REFL; NADD_MUL_LID; NADD_EQ_TRANS; NADD_EQ_SYM])
...
let HREAL_EQ_ADD_LCANCEL = 
    prove((parse_term @"!m n p. (m + n = m + p) <=> (n = p)"), 
          REPEAT GEN_TAC
          |> THEN <| EQ_TAC
          |> THEN <| REWRITE_TAC [HREAL_ADD_LCANCEL]
          |> THEN <| DISCH_THEN SUBST1_TAC
          |> THEN <| REFL_TAC)
...
let HREAL_LE_ADD2 = 
    prove((parse_term @"!a b c d. a <= b /\ c <= d ==> a + c <= b + d"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [HREAL_LE_EXISTS_DEF]
          |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"d1:hreal")) (X_CHOOSE_TAC(parse_term @"d2:hreal")))
...
let TREAL_EQ_TRANS = 
    prove((parse_term @"!x y z. x treal_eq y /\ y treal_eq z ==> x treal_eq z"), 
          REWRITE_TAC [FORALL_PAIR_THM; treal_eq]
          |> THEN <| REPEAT GEN_TAC
          |> THEN <| DISCH_THEN(MP_TAC << MK_COMB << (AP_TERM(parse_term @"(+)") ||>> I) << CONJ_PAIR)
          ...
          |> THEN <| DISCH_THEN(MATCH_ACCEPT_TAC << ONCE_REWRITE_RULE [HREAL_ADD_SYM]))
...
let TREAL_LE_MUL = 
    prove((parse_term @"!x y. (treal_of_num 0) treal_le x /\ (treal_of_num 0) treal_le y
         ==> (treal_of_num 0) treal_le (x treal_mul y)"),
          ...
          |> THEN <| DISCH_THEN(CHOOSE_THEN SUBST1_TAC << MATCH_MP HREAL_LE_EXISTS)
    ...
    prove((parse_term @"!x. ~(x treal_eq treal_of_num 0) ==>
        (treal_inv(x) treal_mul x) treal_eq (treal_of_num 1)"),
          ...
          |> THEN <| MAP_EVERY X_GEN_TAC [(parse_term @"x:hreal");
                                          (parse_term @"y:hreal")]
          |> THEN <| PURE_REWRITE_TAC [treal_eq; treal_of_num; treal_mul; treal_inv]
          ...
          |> THEN <| REWRITE_TAC [HREAL_ADD_LID; HREAL_ADD_RID; HREAL_MUL_LZERO; HREAL_MUL_RZERO]
          |> THENL <| [ALL_TAC;
                       DISJ_CASES_THEN MP_TAC (SPECL [(parse_term @"x:hreal");
...
let TREAL_INV_WELLDEF = 
    prove((parse_term @"!x y. x treal_eq y ==> (treal_inv x) treal_eq (treal_inv y)"), 
          let lemma = 
              prove((parse_term @"(@d. x = x + d) = &0"), 
                    MATCH_MP_TAC SELECT_UNIQUE
                    |> THEN <| BETA_TAC
                    |> THEN <| GEN_TAC
                    |> THEN <| GEN_REWRITE_TAC (funpow 2 LAND_CONV) [GSYM HREAL_ADD_RID]
                    |> THEN <| REWRITE_TAC [HREAL_EQ_ADD_LCANCEL]
          ...
          |> THEN <| DISCH_THEN(MP_TAC << GEN_REWRITE_RULE RAND_CONV [HREAL_ADD_SYM])
          |> THEN <| REWRITE_TAC [HREAL_EQ_ADD_LCANCEL; HREAL_EQ_ADD_RCANCEL]
          |> THEN <| DISCH_TAC
          |> THEN <| ASM_REWRITE_TAC [HREAL_LE_REFL; lemma; HREAL_INV_0; TREAL_EQ_REFL]
          |> THEN <| ASM_CASES_TAC(parse_term @"x2 <= x1")
          |> THEN <| ASM_REWRITE_TAC []
          |> THENL <| [FIRST_ASSUM(ASSUME_TAC << SYM << SELECT_RULE << MATCH_MP HREAL_LE_EXISTS)
                       |> THEN <| UNDISCH_TAC(parse_term @"x1 + y2 = x2 + y1")
                       |> THEN <| FIRST_ASSUM(SUBST1_TAC << SYM)
                       |> THEN <| REWRITE_TAC [HREAL_EQ_ADD_LCANCEL;
                                               GSYM HREAL_ADD_ASSOC]
                       |> THEN <| DISCH_THEN(SUBST1_TAC << SYM)
                       |> THEN <| REWRITE_TAC [ONCE_REWRITE_RULE [HREAL_ADD_SYM] HREAL_LE_ADD]
                       |> THEN 
                       <| GEN_REWRITE_TAC (RAND_CONV << LAND_CONV << RAND_CONV << BINDER_CONV << LAND_CONV) 
                              [HREAL_ADD_SYM]
                       |> THEN <| REWRITE_TAC [HREAL_EQ_ADD_LCANCEL; TREAL_EQ_REFL];
                       DISJ_CASES_THEN MP_TAC (SPECL [(parse_term @"x1:hreal");
                                                      (parse_term @"x2:hreal")] HREAL_LE_TOTAL)
                       |> THEN <| ASM_REWRITE_TAC []
                       |> THEN <| DISCH_TAC
                       |> THEN <| FIRST_ASSUM(ASSUME_TAC << SYM << SELECT_RULE << MATCH_MP HREAL_LE_EXISTS)
                       |> THEN <| UNDISCH_TAC(parse_term @"x1 + y2 = x2 + y1")
                       |> THEN <| FIRST_ASSUM(SUBST1_TAC << SYM)
                       |> THEN <| REWRITE_TAC [HREAL_EQ_ADD_LCANCEL;
                                               GSYM HREAL_ADD_ASSOC]
                       ...
                       |> THEN <| REWRITE_TAC [ONCE_REWRITE_RULE [HREAL_ADD_SYM] HREAL_LE_ADD]
                       |> THEN <| COND_CASES_TAC
                       |> THENL <| [UNDISCH_TAC(parse_term @"(@d. x2 = x1 + d) + y1 <= y1:hreal")
                                    |> THEN <| GEN_REWRITE_TAC (LAND_CONV << RAND_CONV) [GSYM HREAL_ADD_LID]
                                    |> THEN <| REWRITE_TAC [ONCE_REWRITE_RULE [HREAL_ADD_SYM] HREAL_LE_ADD_LCANCEL]
                                    |> THEN <| DISCH_TAC
                                    |> THEN <| SUBGOAL_THEN (parse_term @"(@d. x2 = x1 + d) = &0") MP_TAC
                                    |> THENL <| [ASM_REWRITE_TAC [GSYM HREAL_LE_ANTISYM]
                                                 ...
                                                 |> THEN <| ASM_REWRITE_TAC [HREAL_ADD_RID]];
                                    GEN_REWRITE_TAC (funpow 3 RAND_CONV << BINDER_CONV << LAND_CONV) [HREAL_ADD_SYM]
                                    |> THEN <| REWRITE_TAC [HREAL_EQ_ADD_LCANCEL; TREAL_EQ_REFL]]])
    ...
    prove
       ...
       ==> ?M. (!x. P x ==> x <= M) /\
         ...
         <| [MATCH_MP_TAC REAL_COMPLETE_SOMEPOS
             ...
             |> THEN <| REWRITE_TAC [REAL_ADD_LID]
             |> THEN <| DISCH_TAC
             |> THEN <| MP_TAC(SPEC (parse_term @"\y. P(y + x) :bool") REAL_COMPLETE_SOMEPOS)
             |> THEN <| BETA_TAC
             |> THEN <| W(C SUBGOAL_THEN MP_TAC << funpow 2 (fst << Choice.get << dest_imp) << snd)
             |> THENL <| [CONJ_TAC
                          |> THENL <| [EXISTS_TAC(parse_term @"&0")
                                       ...
                                       |> THEN <| DISCH_THEN(ANTE_RES_THEN MP_TAC)
                                       |> THEN 
                                       <| DISCH_THEN(MP_TAC << SPEC(parse_term @"--x") << MATCH_MP REAL_LE_LADD_IMP)
                                       |> THEN <| DISCH_THEN(MP_TAC << ONCE_REWRITE_RULE [REAL_ADD_SYM])
                          ...
                          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"B:real") STRIP_ASSUME_TAC)]
             |> THEN <| EXISTS_TAC(parse_term @"B + x")
             |> THEN <| CONJ_TAC
             |> THENL 
             <| [GEN_TAC
                 |> THEN <| GEN_REWRITE_TAC (LAND_CONV << RAND_CONV) [lemma]
                 |> THEN <| DISCH_THEN(ANTE_RES_THEN(MP_TAC << SPEC(parse_term @"x:real") << MATCH_MP REAL_LE_LADD_IMP))
                 |> THEN <| ONCE_REWRITE_TAC [REAL_ADD_SYM]
                 ...
                 |> THENL <| [FIRST_ASSUM MATCH_MP_TAC
                              ...
                              |> THENL <| [FIRST_ASSUM MATCH_MP_TAC
                                           ...
                                           |> THEN <| REWRITE_TAC [ONCE_REWRITE_RULE [REAL_ADD_SYM] REAL_ADD_LINV]
                                           |> THEN <| REWRITE_TAC [ONCE_REWRITE_RULE [REAL_ADD_SYM] REAL_ADD_LID]];
                              DISCH_THEN(MP_TAC << SPEC(parse_term @"x:real") << MATCH_MP REAL_LE_LADD_IMP)
                              |> THEN <| MATCH_MP_TAC EQ_IMP
                              |> THEN <| BINOP_TAC
                              |> THENL <| [MATCH_ACCEPT_TAC REAL_ADD_SYM;
