...
let PRE = 
    new_recursive_definition num_RECURSION (parse_term @"(PRE 0 = 0) /\
   (!n. PRE (SUC n) = n)")
...
let ONE = 
    prove((parse_term @"1 = SUC 0"), 
          REWRITE_TAC [BIT1;
                       REWRITE_RULE [NUMERAL] ADD_CLAUSES;
                       NUMERAL])

let TWO = 
    prove((parse_term @"2 = SUC 1"), 
          REWRITE_TAC [BIT0;
                       BIT1;
                       REWRITE_RULE [NUMERAL] ADD_CLAUSES;
                       NUMERAL])
...
let EXP = 
    new_recursive_definition num_RECURSION (parse_term @"(!m. m EXP 0 = 1) /\
...
let LE = 
    new_recursive_definition num_RECURSION (parse_term @"(!m. (m <= 0) <=> (m = 0)) /\
   (!m n. (m <= SUC n) <=> (m = SUC n) \/ (m <= n))")

let LT = 
    new_recursive_definition num_RECURSION (parse_term @"(!m. (m < 0) <=> F) /\
   (!m n. (m < SUC n) <=> (m = n) \/ (m < n))")
...
let LE_ANTISYM = 
    prove((parse_term @"!m n. (m <= n /\ n <= m) <=> (m = n)"), 
          REPEAT INDUCT_TAC
...
let LT_NZ = 
    prove((parse_term @"!n. 0 < n <=> ~(n = 0)"), 
          INDUCT_TAC
          |> THEN <| ASM_REWRITE_TAC [NOT_SUC; LT; EQ_SYM_EQ]
          |> THEN <| CONV_TAC TAUT)
...
let LE_EXISTS = 
    prove((parse_term @"!m n. (m <= n) <=> (?d. n = m + d)"), 
          GEN_TAC
          |> THEN <| INDUCT_TAC
          |> THEN <| ASM_REWRITE_TAC [LE]
          |> THENL <| [REWRITE_TAC [CONV_RULE (LAND_CONV SYM_CONV) (SPEC_ALL ADD_EQ_0)]
                       ...
                       |> THENL <| [DISCH_THEN(DISJ_CASES_THEN2 SUBST1_TAC MP_TAC)
                                    |> THENL <| [EXISTS_TAC(parse_term @"0")
                                                 ...
                                                 DISCH_THEN(X_CHOOSE_THEN (parse_term @"d:num") SUBST1_TAC)
...
let LT_EXISTS = 
    prove((parse_term @"!m n. (m < n) <=> (?d. n = m + SUC d)"), 
          GEN_TAC
          |> THEN <| INDUCT_TAC
          |> THEN <| REWRITE_TAC [LT;
                                  ADD_CLAUSES;
...
let LE_ADD2 = 
    prove((parse_term @"!m n p q. m <= p /\ n <= q ==> m + n <= p + q"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [LE_EXISTS]
          |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"a:num")) (X_CHOOSE_TAC(parse_term @"b:num")))
...
let LE_MULT2 = 
    prove((parse_term @"!m n p q. m <= n /\ p <= q ==> m * p <= n * q"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [LE_EXISTS]
          |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"a:num")) (X_CHOOSE_TAC(parse_term @"b:num")))
...
let LT_LMULT = 
    prove((parse_term @"!m n p. ~(m = 0) /\ n < p ==> m * n < m * p"), 
          REPEAT GEN_TAC
...
let LE_MULT_LCANCEL = 
    prove((parse_term @"!m n p. (m * n) <= (m * p) <=> (m = 0) \/ n <= p"), 
          REPEAT INDUCT_TAC
          |> THEN <| ASM_REWRITE_TAC [MULT_CLAUSES; ADD_CLAUSES; LE_REFL; LE_0; NOT_SUC]
...
let LT_MULT_LCANCEL = 
    prove((parse_term @"!m n p. (m * n) < (m * p) <=> ~(m = 0) /\ n < p"), 
          REPEAT INDUCT_TAC
          |> THEN <| ASM_REWRITE_TAC [MULT_CLAUSES; ADD_CLAUSES; LT_REFL; LT_0; NOT_SUC]
...
let num_WF = 
    prove((parse_term @"!P. (!n. (!m. m < n ==> P m) ==> P n) ==> !n. P n"), 
          GEN_TAC
          |> THEN <| MP_TAC(SPEC (parse_term @"\n. !m. m < n ==> P m") num_INDUCTION)
          |> THEN <| REWRITE_TAC [LT; BETA_THM]
          |> THEN <| MESON_TAC [LT])

let num_WOP = 
    prove((parse_term @"!P. (?n. P n) <=> (?n. P(n) /\ !m. m < n ==> ~P(m))"), 
          GEN_TAC
          |> THEN <| EQ_TAC
          |> THENL <| [ALL_TAC;
                       MESON_TAC []]
...
let num_MAX = 
    prove((parse_term @"!P. (?x. P x) /\ (?M. !x. P x ==> x <= M) <=>
        ?m. P m /\ (!x. P x ==> x <= m)"),
          GEN_TAC
          |> THEN <| EQ_TAC
          |> THENL <| [DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"a:num")) MP_TAC)
                       |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"m:num") MP_TAC << ONCE_REWRITE_RULE [num_WOP])
                       |> THEN <| DISCH_THEN(fun th -> EXISTS_TAC(parse_term @"m:num")
                                                       |> THEN <| MP_TAC th)
                       |> THEN <| REWRITE_TAC [TAUT(parse_term @"(a /\ b ==> c /\ a) <=> (a /\ b ==> c)")]
                       |> THEN <| SPEC_TAC((parse_term @"m:num"), (parse_term @"m:num"))
                       |> THEN <| INDUCT_TAC
                       |> THENL <| [REWRITE_TAC [LE; LT]
                                    |> THEN <| DISCH_THEN(IMP_RES_THEN SUBST_ALL_TAC)
                                    |> THEN <| POP_ASSUM ACCEPT_TAC;
                                    DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC (MP_TAC << SPEC(parse_term @"m:num")))
                                    |> THEN <| REWRITE_TAC [LT]
                                    |> THEN <| CONV_TAC CONTRAPOS_CONV
                                    |> THEN <| DISCH_TAC
                                    |> THEN <| REWRITE_TAC []
...
let EVEN = 
    new_recursive_definition num_RECURSION (parse_term @"(EVEN 0 <=> T) /\
    (!n. EVEN (SUC n) <=> ~(EVEN n))")

let ODD = 
    new_recursive_definition num_RECURSION (parse_term @"(ODD 0 <=> F) /\
    (!n. ODD (SUC n) <=> ~(ODD n))")
...
let EVEN_OR_ODD = 
    prove((parse_term @"!n. EVEN n \/ ODD n"), 
          INDUCT_TAC
          |> THEN <| REWRITE_TAC [EVEN; ODD; NOT_EVEN; NOT_ODD]
...
let EVEN_ADD = 
    prove((parse_term @"!m n. EVEN(m + n) <=> (EVEN m <=> EVEN n)"), 
          INDUCT_TAC
          |> THEN <| ASM_REWRITE_TAC [EVEN; ADD_CLAUSES]
          |> THEN <| X_GEN_TAC(parse_term @"p:num")
          |> THEN <| DISJ_CASES_THEN MP_TAC (SPEC (parse_term @"n:num") EVEN_OR_ODD)
          |> THEN <| DISJ_CASES_THEN MP_TAC (SPEC (parse_term @"p:num") EVEN_OR_ODD)
...
let ODD_MULT = 
    prove((parse_term @"!m n. ODD(m * n) <=> ODD(m) /\ ODD(n)"), 
          REPEAT GEN_TAC
          |> THEN <| REWRITE_TAC [GSYM NOT_EVEN;
                                  EVEN_MULT]
          |> THEN <| CONV_TAC ITAUT)
...
let EVEN_DOUBLE = 
    prove((parse_term @"!n. EVEN(2 * n)"), 
          GEN_TAC
          |> THEN <| REWRITE_TAC [EVEN_MULT]
          |> THEN <| DISJ1_TAC
          |> THEN <| PURE_REWRITE_TAC [BIT0_THM; BIT1_THM]
...
let EVEN_EXISTS_LEMMA = 
    prove((parse_term @"!n. (EVEN n ==> ?m. n = 2 * m) /\
        (~EVEN n ==> ?m. n = SUC(2 * m))"),
          INDUCT_TAC
          ...
          |> THENL <| [EXISTS_TAC(parse_term @"0")
                       ...
                       |> THEN <| CONJ_TAC
                       |> THEN <| DISCH_THEN(ANTE_RES_THEN(X_CHOOSE_TAC(parse_term @"m:num")))
...
let EVEN_ODD_DECOMPOSITION = 
    prove((parse_term @"!n. (?k m. ODD m /\ (n = 2 EXP k * m)) <=> ~(n = 0)"), 
          MATCH_MP_TAC num_WF
          ...
          |> THENL <| [REWRITE_TAC [MULT_CLAUSES; LT]
                       ...
                       ALL_TAC]
          |> THEN <| ANTS_TAC
          |> THENL <| [GEN_REWRITE_TAC LAND_CONV [GSYM(el 2 (CONJUNCTS MULT_CLAUSES))]
...
let SUB = 
    new_recursive_definition num_RECURSION (parse_term @"(!m. m - 0 = m) /\
   (!m n. m - (SUC n) = PRE(m - n))")
...
let LEFT_SUB_DISTRIB = 
    prove((parse_term @"!m n p. m * (n - p) = m * n - m * p"), 
          REPEAT GEN_TAC
          |> THEN <| CONV_TAC SYM_CONV
          |> THEN <| DISJ_CASES_TAC(SPECL [(parse_term @"n:num");
                                           (parse_term @"p:num")] LE_CASES)
          |> THENL <| [FIRST_ASSUM(fun th -> REWRITE_TAC [REWRITE_RULE [GSYM SUB_EQ_0] th])
                       |> THEN <| ASM_REWRITE_TAC [MULT_CLAUSES; SUB_EQ_0; LE_MULT_LCANCEL];
...
let EVEN_SUB = 
    prove((parse_term @"!m n. EVEN(m - n) <=> m <= n \/ (EVEN(m) <=> EVEN(n))"), 
          REPEAT GEN_TAC
          ...
          |> THEN <| ASM_SIMP_TAC []
          |> THEN <| FIRST_ASSUM(MP_TAC << AP_TERM(parse_term @"EVEN") << MATCH_MP SUB_ADD)
...
let FACT = 
    new_recursive_definition num_RECURSION (parse_term @"(FACT 0 = 1) /\
    (!n. FACT (SUC n) = (SUC n) * FACT(n))")

let FACT_LT = 
    prove((parse_term @"!n. 0 < FACT n"), 
          INDUCT_TAC
          |> THEN <| ASM_REWRITE_TAC [FACT; LT_MULT]
...
let FACT_NZ = 
    prove((parse_term @"!n. ~(FACT n = 0)"), 
          REWRITE_TAC [GSYM LT_NZ;
                       FACT_LT])

let FACT_MONO = 
    prove((parse_term @"!m n. m <= n ==> FACT m <= FACT n"), 
          REPEAT GEN_TAC
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"d:num") SUBST1_TAC << REWRITE_RULE [LE_EXISTS])
...
let LT_EXP = 
    prove((parse_term @"!x m n. x EXP m < x EXP n <=> 2 <= x /\ m < n \/
          ...
          |> THENL <| [REWRITE_TAC [GSYM NOT_LT;
                       ...
                       |> THEN <| SPEC_TAC((parse_term @"m:num"), (parse_term @"m:num"))
                       |> THEN <| INDUCT_TAC
                       |> THEN <| REWRITE_TAC [EXP; MULT_CLAUSES; NOT_SUC; LT_REFL; LT]
                       |> THEN <| REWRITE_TAC [ONE; LT_0];
                       ALL_TAC]
          |> THEN <| EQ_TAC
          |> THENL <| [CONV_TAC CONTRAPOS_CONV
                       ...
                       |> THEN 
                       <| FIRST_ASSUM(X_CHOOSE_THEN (parse_term @"d:num") SUBST1_TAC << REWRITE_RULE [LE_EXISTS])
                       |> THEN <| SPEC_TAC((parse_term @"d:num"), (parse_term @"d:num"))
                       |> THEN <| INDUCT_TAC
                       ...
                       STRIP_TAC
                       |> THEN <| FIRST_ASSUM(X_CHOOSE_THEN (parse_term @"d:num") SUBST1_TAC << REWRITE_RULE [LT_EXISTS])
                       |> THEN <| SPEC_TAC((parse_term @"d:num"), (parse_term @"d:num"))
                       |> THEN <| INDUCT_TAC
                       ...
                       |> THENL <| [MATCH_MP_TAC LTE_TRANS
                                    ...
                                    |> THEN <| CONJ_TAC
                                    |> THENL <| [ASM_REWRITE_TAC [MULT_2; LT_ADD; EXP_LT_0];
                                                 ASM_REWRITE_TAC [LE_MULT_RCANCEL]];
                                    MATCH_MP_TAC LTE_TRANS
                                    |> THEN <| EXISTS_TAC(parse_term @"x EXP (m + SUC d)")
                                    |> THEN <| ASM_REWRITE_TAC []
                                    |> THEN <| REWRITE_TAC [ADD_CLAUSES; EXP; MULT_ASSOC; LE_MULT_RCANCEL]
                                    |> THEN <| DISJ1_TAC
                                    |> THEN <| MATCH_MP_TAC LE_TRANS
...
let EXP_MONO_LT_IMP = 
    prove((parse_term @"!x y n. x < y /\ ~(n = 0) ==> x EXP n < y EXP n"), 
          GEN_TAC
          |> THEN <| GEN_TAC
          |> THEN <| INDUCT_TAC
          ...
          |> THEN <| ASM_SIMP_TAC [LT_IMP_LE; LE_MULT_LCANCEL; LT_MULT_RCANCEL; EXP_MONO_LE_IMP; EXP_EQ_0]
...
let DIVMOD_EXIST = 
    prove((parse_term @"!m n. ~(n = 0) ==> ?q r. (m = q * n + r) /\ r < n"), 
          REPEAT STRIP_TAC
          ...
          |> THEN <| DISCH_THEN(CONJUNCTS_THEN2 (X_CHOOSE_TAC(parse_term @"q:num")) MP_TAC)
          ...
          |> THEN <| DISCH_THEN(X_CHOOSE_THEN (parse_term @"d:num") SUBST_ALL_TAC << REWRITE_RULE [LE_EXISTS])
          ...
          |> THEN <| ASM_REWRITE_TAC [GSYM NOT_LE; LE])

let DIVMOD_EXIST_0 = 
    prove((parse_term @"!m n. ?q r. if n = 0 then q = 0 /\ r = m
                else m = q * n + r /\ r < n"),
          REPEAT GEN_TAC
...
let DIVMOD_UNIQ_LEMMA = 
    prove((parse_term @"!m n q1 r1 q2 r2. ((m = q1 * n + r1) /\ r1 < n) /\
          ...
          |> THENL <| [UNDISCH_TAC(parse_term @"m = q2 * n + r2")
                       |> THEN <| ASM_REWRITE_TAC []
                       |> THEN <| DISJ_CASES_THEN MP_TAC (SPECL [(parse_term @"q1:num");
                       ...
                       |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB;
                                               ...
                                               EQ_ADD_LCANCEL]
                       |> THENL <| [DISCH_TAC
                                    |> THEN <| UNDISCH_TAC(parse_term @"r1 < n");
                                    DISCH_THEN(ASSUME_TAC << SYM)
                                    |> THEN <| UNDISCH_TAC(parse_term @"r2 < n")]
                       |> THEN <| ASM_REWRITE_TAC []
                       ...
                       |> THEN <| REWRITE_TAC []
                       |> THEN <| CONV_TAC SYM_CONV
                       |> THEN <| UNDISCH_TAC(parse_term @"m = q1 * n + r2")
                       |> THEN <| ASM_REWRITE_TAC [EQ_ADD_RCANCEL; EQ_MULT_RCANCEL]
                       |> THEN <| REPEAT(UNDISCH_TAC(parse_term @"r2 < n"))
                       |> THEN <| ASM_CASES_TAC(parse_term @"n = 0")
                       |> THEN <| ASM_REWRITE_TAC [GSYM NOT_LE;
                                                   LE_0]])

let DIVMOD_UNIQ = 
    prove((parse_term @"!m n q r. (m = q * n + r) /\ r < n ==> (m DIV n = q) /\ (m MOD n = r)"), 
          REPEAT GEN_TAC
...
let MOD_EQ = 
    prove((parse_term @"!m n p q. (m = n + q * p) ==> (m MOD p = n MOD p)"), 
          REPEAT GEN_TAC
          ...
          |> THENL <| [ASM_REWRITE_TAC [MULT_CLAUSES; ADD_CLAUSES]
                       ...
                       |> THEN <| DISCH_THEN(STRIP_ASSUME_TAC << GSYM << SPEC(parse_term @"n:num"))
                       |> THEN <| ASM_REWRITE_TAC [RIGHT_ADD_DISTRIB;
                                                   GSYM ADD_ASSOC]
                       |> THEN <| MATCH_ACCEPT_TAC ADD_SYM])

let DIV_LE = 
    prove((parse_term @"!m n. ~(n = 0) ==> m DIV n <= m"), 
          REPEAT STRIP_TAC
          |> THEN <| FIRST_ASSUM(fun th -> GEN_REWRITE_TAC RAND_CONV [MATCH_MP DIVISION th])
...
let DIV_MUL_LE = 
    prove((parse_term @"!m n. n * (m DIV n) <= m"), 
          REPEAT GEN_TAC
          |> THEN <| ASM_CASES_TAC(parse_term @"n = 0")
          |> THEN <| ASM_REWRITE_TAC [MULT_CLAUSES; LE_0]
          |> THEN <| POP_ASSUM(MP_TAC << SPEC(parse_term @"m:num") << MATCH_MP DIVISION)
          |> THEN <| DISCH_THEN(fun th -> GEN_REWRITE_TAC RAND_CONV [CONJUNCT1 th])
...
let MOD_MOD = 
    prove((parse_term @"!m n p. ~(n * p = 0) ==> ((m MOD (n * p)) MOD n = m MOD n)"), 
          REPEAT GEN_TAC
...
let MOD_MOD_REFL = 
    prove((parse_term @"!m n. ~(n = 0) ==> ((m MOD n) MOD n = m MOD n)"), 
          REPEAT GEN_TAC
          |> THEN <| DISCH_TAC
          |> THEN <| MP_TAC(SPECL [(parse_term @"m:num");
                                   (parse_term @"n:num");
...
let MOD_EXISTS = 
    prove((parse_term @"!m n. (?q. m = n * q) <=> if n = 0 then (m = 0) else (m MOD n = 0)"), 
          REPEAT GEN_TAC
          |> THEN <| COND_CASES_TAC
          |> THEN <| ASM_REWRITE_TAC [MULT_CLAUSES]
          |> THEN <| EQ_TAC
          ...
          |> THEN <| SUBGOAL_THEN (parse_term @"m = (m DIV n) * n + m MOD n") (fun th -> GEN_REWRITE_TAC LAND_CONV [th])
          |> THENL <| [ASM_MESON_TAC [DIVISION];
                       ASM_REWRITE_TAC [ADD_CLAUSES; MULT_AC]])

let LE_RDIV_EQ = 
    prove((parse_term @"!a b n. ~(a = 0) ==> (n <= b DIV a <=> a * n <= b)"), 
          REPEAT STRIP_TAC
          |> THEN <| EQ_TAC
          |> THEN <| DISCH_TAC
          |> THENL <| [MATCH_MP_TAC LE_TRANS
                       ...
                       SUBGOAL_THEN (parse_term @"a * n < a * (b DIV a + 1)") MP_TAC
                       |> THENL <| [MATCH_MP_TAC LET_TRANS
                                    |> THEN <| EXISTS_TAC(parse_term @"(b DIV a) * a + b MOD a")
                                    |> THEN <| CONJ_TAC
                                    |> THENL <| [ASM_MESON_TAC [DIVISION];
                                                 ALL_TAC]
                                    |> THEN <| SIMP_TAC [LEFT_ADD_DISTRIB; MULT_SYM; MULT_CLAUSES; LT_ADD_LCANCEL]
...
let DIV_MONO = 
    prove((parse_term @"!m n p. ~(p = 0) /\ m <= n ==> m DIV p <= n DIV p"), 
          REPEAT STRIP_TAC
          |> THEN <| MATCH_MP_TAC(MESON [LE_REFL] (parse_term @"(!k:num. k <= a ==> k <= b) ==> a <= b"))
          |> THEN <| ASM_SIMP_TAC [LE_RDIV_EQ]
          |> THEN <| ASM_MESON_TAC [LE_TRANS])

let DIV_MONO_LT = 
    prove((parse_term @"!m n p. ~(p = 0) /\ m + p <= n ==> m DIV p < n DIV p"), 
          REPEAT STRIP_TAC
          |> THEN <| MATCH_MP_TAC(MESON [ADD1; LE_SUC_LT; LE_REFL] (parse_term @"(!k:num. k <= a ==> k + 1 <= b) ==> a < b"))
          |> THEN <| ASM_SIMP_TAC [LE_RDIV_EQ; LEFT_ADD_DISTRIB; MULT_CLAUSES]
          |> THEN <| ASM_MESON_TAC [LE_REFL; LE_TRANS; LE_ADD2; ADD_SYM])

let DIV_EQ_0 = 
    prove((parse_term @"!m n. ~(n = 0) ==> ((m DIV n = 0) <=> m < n)"), 
          REPEAT(STRIP_TAC
                 |> ORELSE <| EQ_TAC)
          |> THENL <| [FIRST_ASSUM(SUBST1_TAC << CONJUNCT1 << SPEC(parse_term @"m:num") << MATCH_MP DIVISION)
                       |> THEN <| ASM_SIMP_TAC [MULT_CLAUSES; ADD_CLAUSES; DIVISION];
                       MATCH_MP_TAC DIV_UNIQ
                       |> THEN <| EXISTS_TAC(parse_term @"m:num")
                       |> THEN <| ASM_REWRITE_TAC [MULT_CLAUSES; ADD_CLAUSES]])

let MOD_EQ_0 = 
    prove((parse_term @"!m n. ~(n = 0) ==> ((m MOD n = 0) <=> (?q. m = q * n))"), 
          REPEAT(STRIP_TAC
                 |> ORELSE <| EQ_TAC)
          |> THENL <| [FIRST_ASSUM(SUBST1_TAC << CONJUNCT1 << SPEC(parse_term @"m:num") << MATCH_MP DIVISION)
                       |> THEN <| ASM_SIMP_TAC [MULT_CLAUSES; ADD_CLAUSES; DIVISION]
...
let ODD_MOD = 
    prove((parse_term @"!n. ODD(n) <=> (n MOD 2 = 1)"), 
          GEN_TAC
          |> THEN <| REWRITE_TAC [GSYM NOT_EVEN;
                                  EVEN_MOD]
...
let MOD_MULT_RMOD = 
    prove((parse_term @"!m n p. ~(n = 0) ==> ((m * (p MOD n)) MOD n = (m * p) MOD n)"), 
          REPEAT STRIP_TAC
          |> THEN <| CONV_TAC SYM_CONV
          |> THEN <| MATCH_MP_TAC MOD_EQ
...
let MOD_EXP_MOD = 
    prove((parse_term @"!m n p. ~(n = 0) ==> (((m MOD n) EXP p) MOD n = (m EXP p) MOD n)"), 
          REPEAT STRIP_TAC
...
let MOD_MULT_ADD = 
    prove((parse_term @"!m n p. (m * n + p) MOD n = p MOD n"), 
          REPEAT STRIP_TAC
          |> THEN <| ASM_CASES_TAC(parse_term @"n = 0")
          |> THEN <| ASM_REWRITE_TAC [MULT_CLAUSES; ADD_CLAUSES]
          |> THEN <| MATCH_MP_TAC MOD_UNIQ
...
let DIV_MULT_ADD = 
    prove((parse_term @"!a b n. ~(n = 0) ==> (a * n + b) DIV n = a + b DIV n"), 
          REPEAT STRIP_TAC
          |> THEN <| MATCH_MP_TAC DIV_UNIQ
...
let MOD_ADD_MOD = 
    prove((parse_term @"!a b n. ~(n = 0) ==> ((a MOD n + b MOD n) MOD n = (a + b) MOD n)"), 
          REPEAT STRIP_TAC
          |> THEN <| CONV_TAC SYM_CONV
          |> THEN <| MATCH_MP_TAC MOD_EQ
          |> THEN <| EXISTS_TAC(parse_term @"a DIV n + b DIV n")
          |> THEN <| REWRITE_TAC [RIGHT_ADD_DISTRIB]
          |> THEN <| ONCE_REWRITE_TAC [AC ADD_AC (parse_term @"(a + b) + (c + d) = (c + a) + (d + b)")]
          |> THEN <| BINOP_TAC
          |> THEN <| ASM_SIMP_TAC [DIVISION])
...
let DIV_ADD_MOD = 
    prove((parse_term @"!a b n. ~(n = 0)
          ...
          |> THEN <| DISCH_THEN(fun th -> 
                         MAP_EVERY (MP_TAC << CONJUNCT1 << C SPEC th) [(parse_term @"a + b:num");
                                                                       (parse_term @"a:num");
                                                                       (parse_term @"b:num")])
          |> THEN <| DISCH_THEN(fun th1 -> DISCH_THEN(fun th2 -> MP_TAC(MK_COMB(AP_TERM (parse_term @"(+)") th2, th1))))
          |> THEN <| DISCH_THEN(fun th -> GEN_REWRITE_TAC (funpow 2 LAND_CONV) [th])
          ...
          |> THEN <| DISCH_THEN(fun th -> 
                         ...
                         |> THEN <| DISCH_TAC
                         |> THEN <| MP_TAC th)
          |> THEN <| ASM_REWRITE_TAC [EQ_ADD_RCANCEL; EQ_ADD_LCANCEL; EQ_MULT_RCANCEL]
...
let MOD_LE = 
    prove((parse_term @"!m n. ~(n = 0) ==> m MOD n <= m"), 
          REPEAT GEN_TAC
          |> THEN <| DISCH_THEN(fun th -> GEN_REWRITE_TAC RAND_CONV [MATCH_MP DIVISION th])
          |> THEN <| ONCE_REWRITE_TAC [ADD_SYM]
...
let DIV_LE_EXCLUSION = 
    prove((parse_term @"!a b c d. ~(b = 0) /\ b * c < (a + 1) * d ==> c DIV d <= a DIV b"), 
          REPEAT GEN_TAC
          ...
          |> THEN <| SUBGOAL_THEN (parse_term @"b * d * k <= b * c ==> (b * k) * d < (a + 1) * d") MP_TAC
...
let MULT_DIV_LE = 
    prove((parse_term @"!m n p. ~(p = 0) ==> m * (n DIV p) <= (m * n) DIV p"), 
          REPEAT GEN_TAC
          |> THEN <| SIMP_TAC [LE_RDIV_EQ]
          |> THEN <| DISCH_THEN(MP_TAC << SPEC(parse_term @"n:num") << MATCH_MP DIVISION)
          |> THEN <| DISCH_THEN(fun th -> GEN_REWRITE_TAC (RAND_CONV << RAND_CONV) [CONJUNCT1 th])
          |> THEN <| REWRITE_TAC [LEFT_ADD_DISTRIB]
          |> THEN <| REWRITE_TAC [MULT_AC; LE_ADD])

let DIV_DIV = 
    prove((parse_term @"!m n p. ~(n * p = 0) ==> ((m DIV n) DIV p = m DIV (n * p))"), 
          REWRITE_TAC [MULT_EQ_0; DE_MORGAN_THM]
          |> THEN <| REPEAT STRIP_TAC
          |> THEN <| MATCH_MP_TAC(MESON [LE_ANTISYM] (parse_term @"(!k. k <= m <=> k <= n) ==> m = n"))
          |> THEN <| ASM_SIMP_TAC [LE_RDIV_EQ; MULT_EQ_0; MULT_ASSOC])

let DIV_MOD = 
    prove((parse_term @"!m n p. ~(n * p = 0) ==> ((m DIV n) MOD p = (m MOD (n * p)) DIV n)"), 
          REWRITE_TAC [MULT_EQ_0; DE_MORGAN_THM]
          |> THEN <| REPEAT STRIP_TAC
          |> THEN <| MATCH_MP_TAC(MESON [LE_ANTISYM] (parse_term @"(!k. k <= m <=> k <= n) ==> m = n"))
          |> THEN <| X_GEN_TAC(parse_term @"k:num")
          |> THEN <| MATCH_MP_TAC EQ_TRANS
          |> THEN <| EXISTS_TAC(parse_term @"k + p * ((m DIV n) DIV p) <= (m DIV n)")
          |> THEN <| CONJ_TAC
          |> THENL <| [MP_TAC(SPECL [(parse_term @"m DIV n");
                                     (parse_term @"p:num")] DIVISION)
                       |> THEN <| ASM_REWRITE_TAC [];
                       MP_TAC(SPECL [(parse_term @"m:num");
                                     (parse_term @"n * p:num")] DIVISION)
                       |> THEN <| ASM_SIMP_TAC [LE_RDIV_EQ; MULT_EQ_0; DIV_DIV; LEFT_ADD_DISTRIB]]
          |> THEN <| REWRITE_TAC [MULT_AC]
          |> THEN <| MESON_TAC [ADD_SYM; MULT_SYM; LE_ADD_RCANCEL])

let MOD_MOD_EXP_MIN = 
    prove((parse_term @"!x p m n. ~(p = 0)
              ==> x MOD (p EXP m) MOD (p EXP n) = x MOD (p EXP (MIN m n))"),
          REPEAT STRIP_TAC
          |> THEN <| REWRITE_TAC [MIN]
          |> THEN <| ASM_CASES_TAC(parse_term @"m:num <= n")
          |> THEN <| ASM_REWRITE_TAC []
          |> THENL <| [FIRST_X_ASSUM(CHOOSE_THEN SUBST1_TAC << GEN_REWRITE_RULE I [LE_EXISTS])
...
let PRE_ELIM_THM' = 
    prove((parse_term @"P(PRE n) <=> ?m. (n = SUC m \/ m = 0 /\ n = 0) /\ P m"), 
         MP_TAC
            (INST 
                [(parse_term @"\x:num. ~P x"), 
                (parse_term @"P:num->bool")] 
                PRE_ELIM_THM)
        |> THEN <| MESON_TAC [])

let SUB_ELIM_THM = 
    prove((parse_term @"P(a - b) <=> !d. a = b + d \/ a < b /\ d = 0 ==> P d"), 
          DISJ_CASES_TAC(SPECL [(parse_term @"a:num");
                                (parse_term @"b:num")] LTE_CASES)
          |> THENL <| [ASM_MESON_TAC [NOT_LT; SUB_EQ_0; LT_IMP_LE; LE_ADD];
                       ALL_TAC]
          |> THEN <| FIRST_ASSUM(X_CHOOSE_THEN (parse_term @"e:num") SUBST1_TAC << REWRITE_RULE [LE_EXISTS])
...
let NUM_CANCEL_CONV = 
    let rec minter i l1' l2' l1 l2 = 
        ...
        elif l2 = [] then (i, l1 @ l1', l2')
        else 
            let h1 = hd l1
            let h2 = hd l2
            if h1 = h2 then minter (h1 :: i) l1' l2' (tl l1) (tl l2)
            elif h1 < h2 then minter i (h1 :: l1') l2' (tl l1) l2
    ...
    fun tm -> 
        choice {
            ...
            return! GEN_REWRITE_RULE (RAND_CONV << REPEATC) [EQ_ADD_LCANCEL; EQ_ADD_LCANCEL_0; EQ_ADD_LCANCEL_0'] eth
        }
...
let TRANSITIVE_STEPWISE_LT = 
    prove((parse_term @"!R. (!x y z. R x y /\ R y z ==> R x z) /\ (!n. R n (SUC n))
        ==> !m n. m < n ==> R m n"),
          REPEAT GEN_TAC
          |> THEN <| MATCH_MP_TAC(TAUT(parse_term @"(a ==> (c <=> b)) ==> a /\ b ==> c"))
