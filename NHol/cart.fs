...
let DIMINDEX_NONZERO = 
    prove((parse_term @"!s:A->bool. ~(dimindex(s) = 0)"), 
          GEN_TAC
          |> THEN <| REWRITE_TAC [dimindex]
          |> THEN <| COND_CASES_TAC
          |> THEN <| ASM_SIMP_TAC [CARD_EQ_0; ARITH]
          |> THEN <| SET_TAC [])
...
let FINITE_INDEX_INJ = 
    prove((parse_term @"!i j. 1 <= i /\ i <= dimindex(:A) /\
         1 <= j /\ j <= dimindex(:A)
...
let lambda = 
    new_definition(parse_term @"(lambda) g =
...
let finite_sum_tybij = 
    let th = 
        prove((parse_term @"?x. x IN 1..(dimindex(:A) + dimindex(:B))"), 
              EXISTS_TAC(parse_term @"1")
              |> THEN <| SIMP_TAC [IN_NUMSEG;
                                   LE_REFL;
                                   DIMINDEX_GE_1;
...
let pastecart = 
    new_definition(parse_term @"(pastecart:A^M->A^N->A^(M,N)finite_sum) f g =
...
let sndcart = 
    new_definition(parse_term @"(sndcart:A^(M,N)finite_sum->A^N) f =
...
let DIMINDEX_FINITE_SUM = 
    prove
        ((parse_term @"dimindex(:(M,N)finite_sum) = dimindex(:M) + dimindex(:N)"), 
         ...
         |> THEN <| REWRITE_TAC [REWRITE_RULE [HAS_SIZE] DIMINDEX_HAS_SIZE_FINITE_SUM])

let FSTCART_PASTECART = 
    prove((parse_term @"!x y. fstcart(pastecart (x:A^M) (y:A^N)) = x"), 
          SIMP_TAC [pastecart;
                    fstcart;
                    CART_EQ;
                    LAMBDA_BETA;
...
let SNDCART_PASTECART = 
    prove((parse_term @"!x y. sndcart(pastecart (x:A^M) (y:A^N)) = y"), 
          SIMP_TAC [pastecart; sndcart; CART_EQ; LAMBDA_BETA]
          |> THEN <| REPEAT STRIP_TAC
          |> THEN <| W(fun (_, w) gl -> 
                            choice {
                                let! tm1 = lhand w
                                return! MP_TAC(PART_MATCH (Choice.bind lhs << rand) LAMBDA_BETA tm1) gl
                            })
          |> THEN <| ANTS_TAC
          |> THENL <| [REWRITE_TAC [DIMINDEX_FINITE_SUM]
                       |> THEN <| MATCH_MP_TAC(ARITH_RULE(parse_term @"1 <= i /\ i <= b ==> 1 <= i + a /\ i + a <= a + b"))
                       |> THEN <| ASM_REWRITE_TAC [];
                       DISCH_THEN SUBST1_TAC
                       |> THEN <| REWRITE_TAC []
                       |> THEN <| ASM_SIMP_TAC [ADD_SUB;
                                                ARITH_RULE(parse_term @"1 <= i ==> ~(i + a <= a)")]])

let PASTECART_FST_SND = 
    prove((parse_term @"!z. pastecart (fstcart z) (sndcart z) = z"), 
          SIMP_TAC [pastecart; fstcart; sndcart; CART_EQ; LAMBDA_BETA]
          |> THEN <| REPEAT GEN_TAC
          |> THEN <| COND_CASES_TAC
          |> THEN <| ASM_REWRITE_TAC []
          |> THEN <| ASM_SIMP_TAC [DIMINDEX_FINITE_SUM;
                                   LAMBDA_BETA;
...
let define_finite_type = 
    let lemma_pre = 
        prove((parse_term @"~(n = 0) ==> ?x. x IN 1..n"), 
              DISCH_TAC
    ...
    fun n -> 
        choice {
            ...
            let! th0 = INST [n', n_tm] lemma_pre
            let! tm1 = lhand(concl th0)
            let! tm2 = rand tm1
            let th1 = MP (Choice.result th0) (EQF_ELIM(NUM_EQ_CONV tm2))
            return! POST_RULE(new_type_definition ns ("mk_" + ns', "dest_" + ns') th1)
        }
...
let HAS_SIZE_1 = 
    prove((parse_term @"(:1) HAS_SIZE 1"), 
          SUBGOAL_THEN (parse_term @"(:1) = {one}") SUBST1_TAC
          |> THENL <| [REWRITE_TAC [EXTENSION; IN_UNIV; IN_SING]
                       |> THEN <| MESON_TAC [one];
...
let FINITE_CART = 
    prove((parse_term @"!P. (!i. 1 <= i /\ i <= dimindex(:N) ==> FINITE {x | P i x})
       ==> FINITE {v:A^N | !i. 1 <= i /\ i <= dimindex(:N) ==> P i (v$i)}"),
        ...
        ==> FINITE {v:A^N | (!i. 1 <= i /\ i <= dimindex(:N) /\ i <= n
          ...
          |> THEN <| INDUCT_TAC
          |> THENL <| [REWRITE_TAC [ARITH_RULE(parse_term @"1 <= i /\ i <= n /\ i <= 0 <=> F")]
                       |> THEN <| SIMP_TAC [ARITH_RULE(parse_term @"1 <= i /\ i <= n /\ 0 < i <=> 1 <= i /\ i <= n")]
                       |> THEN <| SUBGOAL_THEN 
                                      (parse_term 
                                           @"{v | !i. 1 <= i /\ i <= dimindex (:N) ==> v$i = (@x. F)} = {(lambda i. @x. F):A^N}") 
                                      (fun th -> SIMP_TAC [FINITE_RULES; th])
                       |> THEN <| SIMP_TAC [EXTENSION; IN_SING; IN_ELIM_THM; CART_EQ; LAMBDA_BETA];
                       ALL_TAC]
          |> THEN <| DISCH_TAC
          ...
          |> THENL <| [MATCH_MP_TAC FINITE_IMAGE
                       |> THEN <| ASM_SIMP_TAC [FINITE_PRODUCT;
                                                ARITH_RULE(parse_term @"1 <= SUC n");
                                                ARITH_RULE(parse_term @"SUC n <= m ==> n <= m")];
                       ALL_TAC]
          |> THEN <| REWRITE_TAC [SUBSET; IN_IMAGE; IN_ELIM_PAIR_THM; EXISTS_PAIR_THM]
          |> THEN <| X_GEN_TAC(parse_term @"v:A^N")
          |> THEN <| REWRITE_TAC [IN_ELIM_THM]
          |> THEN <| STRIP_TAC
          |> THEN <| EXISTS_TAC(parse_term @"(v:A^N)$(SUC n)")
          |> THEN <| EXISTS_TAC(parse_term @"(lambda i. if i = SUC n then @x. F else (v:A^N)$i):A^N")
          |> THEN <| SIMP_TAC [CART_EQ;
                               LAMBDA_BETA;
                               ARITH_RULE(parse_term @"i <= n ==> ~(i = SUC n)")]
          |> THEN <| ASM_MESON_TAC [LE;
                                    ARITH_RULE(parse_term @"1 <= SUC n");
...
let HAS_SIZE_CART_UNIV = 
    prove((parse_term @"!m. (:A) HAS_SIZE m ==> (:A^N) HAS_SIZE m EXP (dimindex(:N))"), 
          REPEAT STRIP_TAC
          |> THEN <| SUBGOAL_THEN (parse_term @"(:(N)finite_image->A) HAS_SIZE m EXP (dimindex(:N))") MP_TAC
          |> THENL <| [ASM_SIMP_TAC [HAS_SIZE_FUNSPACE_UNIV; HAS_SIZE_FINITE_IMAGE];
                       DISCH_THEN
                           (MP_TAC << ISPEC(parse_term @"mk_cart:((N)finite_image->A)->A^N") 
                            << MATCH_MP(REWRITE_RULE [IMP_CONJ_ALT] HAS_SIZE_IMAGE_INJ))
...
let HAS_SIZE_PCROSS = 
    prove
        ...
        s HAS_SIZE m /\ t HAS_SIZE n ==> (s PCROSS t) HAS_SIZE (m * n)"),
         ...
         |> THEN <| EXISTS_TAC(parse_term @"\z:A^(M,N)finite_sum. fstcart z,sndcart z")
         |> THEN <| REWRITE_TAC [FORALL_IN_GSPEC; PASTECART_IN_PCROSS]
         |> THEN <| REWRITE_TAC [IN_ELIM_PAIR_THM; PASTECART_FST_SND]
         |> THEN <| REWRITE_TAC [FORALL_IN_PCROSS; FSTCART_PASTECART; SNDCART_PASTECART])
...
let FINITE_PCROSS_EQ = 
    prove((parse_term @"!(s:A^M->bool) (t:A^N->bool).
        FINITE(s PCROSS t) <=> s = {} \/ t = {} \/ FINITE s /\ FINITE t"),
          ...
          |> THEN <| REPEAT STRIP_TAC
          |> THEN <| MATCH_MP_TAC FINITE_SUBSET
          |> THENL <| [EXISTS_TAC(parse_term @"IMAGE fstcart ((s PCROSS t):A^(M,N)finite_sum->bool)");
                       EXISTS_TAC(parse_term @"IMAGE sndcart ((s PCROSS t):A^(M,N)finite_sum->bool)")]
          |> THEN <| ASM_SIMP_TAC [FINITE_IMAGE; SUBSET; IN_IMAGE; EXISTS_PASTECART]
          |> THEN <| REWRITE_TAC [PASTECART_IN_PCROSS; FSTCART_PASTECART; SNDCART_PASTECART]
          |> THEN <| ASM SET_TAC [])
