...
let INJ_INVERSE2 = 
  ...
  prove((parse_term @"!P:A->B->C.
   ...
   |> THEN <| REFL_TAC)
#else
    Choice1Of2 <| Sequent([], parse_term @"!P. (!x1 y1 x2 y2. P x1 y1 = P x2 y2 <=> x1 = x2 /\ y1 = y2)
...
#else
    Choice1Of2 <| Sequent([], parse_term @"!x1 y1 x2 y2. NUMPAIR x1 y1 = NUMPAIR x2 y2 <=> x1 = x2 /\ y1 = y2") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([], parse_term @"!b1 x1 b2 x2. NUMSUM b1 x1 = NUMSUM b2 x2 <=> (b1 <=> b2) /\ x1 = x2") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([], parse_term @"!n1 n2. INJN n1 = INJN n2 <=> n1 = n2") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([], parse_term @"!f1 f2. INJF f1 = INJF f2 <=> f1 = f2") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([], parse_term @"!f1 f1' f2 f2'. INJP f1 f2 = INJP f1' f2' <=> f1 = f1' /\ f2 = f2'") : thm
#endif
...
let MK_REC_INJ = 
    ...
    prove((parse_term @"!x y. (_mk_rec x :(A)recspace = _mk_rec y)
        ...
        |> THEN <| ASM_REWRITE_TAC [])
#else
    Choice1Of2 <| Sequent([], parse_term @"!x y. _mk_rec x = _mk_rec y ==> ZRECSPACE x /\ ZRECSPACE y ==> x = y") : thm
#endif
;;
...
#else
    Choice1Of2 <| Sequent([], parse_term @"!x y. _dest_rec x = _dest_rec y <=> x = y") : thm
#endif
;;
    ...
    prove
        ((parse_term @"!c i r. ~(CONSTR c i r :(A)recspace = BOTTOM)"), 
         ...
         |> THEN <| REWRITE_TAC [fst recspace_tydef
                                 snd recspace_tydef])
#else
    Choice1Of2 <| Sequent([], parse_term @"!c i r. ~(CONSTR c i r = BOTTOM)") : thm
#endif
;;
...
#else
    Choice1Of2 <| Sequent([], parse_term @"!c1 i1 r1 c2 i2 r2.
    ...
    prove((parse_term @"!P. P(BOTTOM) /\
      ...
      |> THENL <| [REPEAT GEN_TAC
                   ...
                   |> THEN <| REWRITE_TAC [fst recspace_tydef
                                           snd recspace_tydef]])
#else
    Choice1Of2 <| Sequent([], parse_term @"!P. P BOTTOM /\ (!c i r. (!n. P (r n)) ==> P (CONSTR c i r))
         ==> (!x. P x)") : thm
#endif
;;
...
let CONSTR_REC = 
  ...
  prove((parse_term @"!Fn:num->A->(num->(A)recspace)->(num->B)->B.
    ...
    |> THENL <| [W(MP_TAC << PART_MATCH Choice.get <| rand CONSTR_IND << snd)
                 ...
                 |> THEN <| REWRITE_TAC [BETA_THM]])
#else
    Choice1Of2 <| Sequent([], parse_term @"!Fn. ?f. !c i r. f (CONSTR c i r) = Fn c i r (\n. f (r n))") : thm
#endif
...
#else
    Choice1Of2 <| Sequent([],parse_term @"!f:num->A. f = FCONS (f 0) (f << SUC)") : thm
#endif
...
let define_type_raw_001 = 
    ...
    let prove_model_inhabitation rth = 
        ...
        let concs = map concl bases @ map (Choice.get << rand << concl) imps
        ...
        let rec exhaust_inhabitations ths sofar = 
            ...
            let useful = 
                filter 
                    (fun th -> not(mem (fst(strip_comb(Choice.get <| rand(concl th)))) dunnit)) 
                    ths
            if useful = []
            then sofar
            else 
    ...
    let define_inductive_type cdefs exth = 
        ...
        let bij2a = AP_THM th2 (Choice.get <| rand(Choice.get <| rand(concl bij2)))
    ...
    let pullback_induction_clause tybijpairs conthms = 
        ...
        fun rthm tm -> 
            ...
            then 
                ...
                let contm1 = Choice.get <| rand(concl conth1)
                let conth2 = 
                    TRANS conth1 
    ...
    let create_recursive_functions tybijpairs consindex conthms rth = 
        ...
        let cleanup_fxthm cth fxth = 
            let tms = snd(strip_comb(Choice.get <| rand(Choice.get <| rand(concl fxth))))
    ...
    let create_recursion_iso_constructor = 
        ...
        let rec extract_arg tup v = 
            ...
            then REFL tup
                ...
                let tup' = Choice.get <| rand(concl PAIR_th)
                if tup' = v
                then PAIR_th
                else 
                    let th = extract_arg (Choice.get <| rand(concl PAIR_th)) v
                    SUBS [SYM PAIR_th] th
        fun consindex -> 
    ...
    let derive_recursion_theorem = 
        ...
        fun tybijpairs consindex conthms rath -> 
            ...
            let betm = 
                let v, bod = Choice.get <| dest_abs(Choice.get <| rand(concl eth))
            ...
            let hackdown_rath th = 
                ...
                let th3 = 
                    TRANS th2 (funpow 2 RATOR_CONV BETA_CONV (Choice.get <| rand(concl th2)))
                let th4 = TRANS th3 (RATOR_CONV BETA_CONV (Choice.get <| rand(concl th3)))
                let th5 = TRANS th4 (BETA_CONV(Choice.get <| rand(concl th4)))
    ...
    fun def -> 
        ...
        let conthms = 
            map (fun th -> 
                    let args = fst(strip_abs(Choice.get <| rand(concl th)))
    ...
    let DE_EXISTENTIALIZE_RULE = 
        ...
        let rec DE_EXISTENTIALIZE_RULE th = 
            ...
            then [], th
                ...
                let th1 = USE_PTH th
                let v1 = Choice.get <| rand(Choice.get <| rand(concl th1))
